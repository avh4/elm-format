...
showPromptMessage (FilesWillBeOverwritten filePaths) = unlines
...
showErrorMessage (BadInputFiles filePaths) = unlines
...
showErrorMessage SingleOutputWithMultipleInputs = unlines
...
showErrorMessage (MustSpecifyVersionWithUpgrade elmVersion) =
  "I can only upgrade code to specific Elm versions.  To make sure I'm doing what you expect, you must also specify --elm-version="
    ++ show elmVersion
    ++ " when you use --upgrade."

showErrorMessage NoInputs =
  error
    "Error case NoInputs should be handled elsewhere.  Please report this issue at https://github.com/avh4/elm-format/issues"
