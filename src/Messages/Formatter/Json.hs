module Messages.Formatter.Json
  ( format
  , init
  , done
  )
...
format elmVersion autoYes infoFormatter = case infoFormatter of
  OnInfo  info    next -> showInfo elmVersion info next

  Approve _prompt next -> case autoYes of
...
showInfo _ (ProcessingFile _) next = return next
