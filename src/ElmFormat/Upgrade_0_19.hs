...
transform exposed importAliases expr =
  let basicsReplacements = Dict.fromList
        [ ( "flip"
          , Lambda
            [makeArg "f", makeArg "b", makeArg "a"]
            []
            (noRegion $ App (makeVarRef "f")
        ...
        , ( "rem"
          , Lambda
            ...
            (noRegion $ App
              ...
              [([], makeVarRef "divisor"), ([], makeVarRef "dividend")]
              (FAJoinFirst JoinAll)
            )
            False
          )
      ...
      replace var = case var of
        ...
        OpRef (SymbolIdentifier "!") -> Just $ Lambda
          [makeArg "model", makeArg "cmds"]
          []
          (noRegion $ Binops (makeVarRef "model")
                             [([], var, [], makeVarRef "cmds")]
                             False
          )
          False
        ...
        OpRef (SymbolIdentifier "%") -> Just $ Lambda
          ...
          (noRegion $ App
            (makeVarRef "modBy")
            [([], makeVarRef "modulus"), ([], makeVarRef "dividend")]
            (FAJoinFirst JoinAll)
          )
          False
      ...
      makeTuple n =
          let vars = if n <= 26
                then fmap (\c -> [c]) (take n ['a' .. 'z'])
                  ...
                  (pleaseReport'' "UNEXPECTED TUPLE" "more than 26 elements")
          in  Lambda
                (fmap makeArg vars)
                []
                (noRegion $ AST.Expression.Tuple
                  (fmap (\v -> Commented [] (makeVarRef v) []) vars)
                  False
                )
                False
  in  case RA.drop expr of
        VarExpr var -> Maybe.fromMaybe expr $ fmap noRegion $ replace var

        App (A _ (VarExpr var)) args multiline ->
          Maybe.fromMaybe expr
            $ fmap (\new -> applyLambda (noRegion new) args multiline)
            $ replace var

        TupleFunction n -> noRegion $ makeTuple n
        ...
        ExplicitList terms' trailing multiline ->
          let ha = (fmap UppercaseIdentifier ["Html", "Attributes"])
              styleExposed =
                  Dict.lookup (LowercaseIdentifier "style") exposed == Just ha
              haAlias = Dict.lookup ha importAliases
          in  noRegion $ ExplicitList
                (concat $ fmap (expandHtmlStyle styleExposed haAlias) $ terms')
                trailing
                multiline
...
expandHtmlStyle
  :: Bool
  -> Maybe UppercaseIdentifier
  -> (Comments, PreCommented (WithEol Expr))
  -> [(Comments, PreCommented (WithEol Expr))]
expandHtmlStyle styleExposed importAlias (preComma, (pre, (term, eol))) =
  let
    lambda fRef = Lambda
      [([], noRegion $ AST.Pattern.Tuple [makeArg' "a", makeArg' "b"])]
      []
      (noRegion $ App (noRegion $ VarExpr $ fRef)
                      [([], makeVarRef "a"), ([], makeVarRef "b")]
                      (FAJoinFirst JoinAll)
      )
      False

    isHtmlAttributesStyle var = case var of
      VarRef [UppercaseIdentifier "Html", UppercaseIdentifier "Attributes"] (LowercaseIdentifier "style")
        -> True
      VarRef [alias] (LowercaseIdentifier "style")
        | Just alias == importAlias -> True
      VarRef [] (LowercaseIdentifier "style") -> styleExposed
      _ -> False
  in
    case RA.drop term of
      App (A _ (VarExpr var)) [(preStyle, A _ (ExplicitList styles trailing _))] _
        | isHtmlAttributesStyle var
        -> fmap
          (\(preComma', (pre', (style, eol'))) ->
            ( preComma ++ preComma'
            , ( pre
                ++ preStyle
                ++ pre'
              ...
              , ( applyLambda (noRegion $ lambda var)
                              [([], style)]
                              (FAJoinFirst JoinAll)
                , eol'
                )
...
makeArg varName = ([], noRegion $ VarPattern $ LowercaseIdentifier varName)
...
inlineVar' name insertMultiline value expr = case expr of
  VarExpr (VarRef [] n) | n == name -> Just value

  AST.Expression.Tuple terms' multiline ->
    let step (acc, expand) t@(Commented pre (A _ term) post) =
            case inlineVar' name insertMultiline value term of
              Nothing -> (ReversedList.push t acc, expand)
              Just term' ->
                ( ReversedList.push (Commented pre (noRegion term') post) acc
                , insertMultiline || expand
                )

        (terms'', multiline'') =
            foldl step (ReversedList.empty, multiline) terms'
    in  Just $ AST.Expression.Tuple (ReversedList.toList terms'') multiline''
...
applyLambda
  :: Expr -> [PreCommented Expr] -> FunctionApplicationMultiline -> Expr
applyLambda lambda args appMultiline =
  let
    getMapping
      :: PreCommented Pattern
      -> PreCommented Expr
      -> Maybe [(LowercaseIdentifier, Expr')]
    getMapping pat arg = case (pat, arg) of
      ((preVar, A _ (VarPattern name)), (preArg, arg')) ->
        Just [(name, Parens $ Commented (preVar ++ preArg) arg' [])]

      ((preVar, A _ (AST.Pattern.Tuple [Commented preA (A _ (VarPattern nameA)) postA, Commented preB (A _ (VarPattern nameB)) postB])), (preArg, A _ (AST.Expression.Tuple [Commented preAe eA postAe, Commented preBe eB postBe] _)))
        -> Just
          [ ( nameA
            , Parens $ Commented
              (preVar ++ preArg)
              (noRegion $ Parens $ Commented (preA ++ preAe)
                                             eA
                                             (postAe ++ postA)
              )
              []
    ...
    case (RA.drop lambda, args) of
      (Lambda (pat : restVar) preBody body multiline, arg : restArgs) ->
        case getMapping pat arg of
          ...
          Just mappings ->
            let newBody = foldl
                  (\e (name, value) -> mapExpr
                    (inlineVar name (appMultiline == FASplitFirst) value)
                    e
                  )
                  body
                  mappings
                newMultiline = case appMultiline of
            ...
            in  case restVar of
                  [] ->
                      -- we applied the argument and none are left, so remove the lambda
                        noRegion $ App
                    (noRegion $ Parens $ Commented preBody newBody [])
                    restArgs
                    newMultiline
                  _ : _ ->
                      -- we applied this argument; try to apply the next argument
                           applyLambda
                    (noRegion $ Lambda restVar preBody newBody multiline)
                    restArgs
                    newMultiline
