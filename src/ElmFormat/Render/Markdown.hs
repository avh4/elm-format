...
formatMarkdown formatCode blocks =
  let
    blocks'            = toList blocks

    needsInitialBlanks = case blocks' of
      (Para inlines : _) -> case toList inlines of
        (Str a : Str b : _) ->
    ...
    needsTrailingBlanks = case blocks' of
      []       -> False
      (_ : []) -> needsInitialBlanks
      _        -> True
  in
    formatMarkdown' formatCode
                    False
                    needsInitialBlanks
                    needsTrailingBlanks
                    blocks'
...
mapWithPrev f (first : rest) =
  f Nothing first
    : zipWith (\prev next -> f (Just prev) next) (first : rest) rest


formatMarkdown'
  :: (String -> Maybe String) -> Bool -> Bool -> Bool -> [Block] -> String
formatMarkdown' formatCode isListItem needsInitialBlanks needsTrailingBlanks blocks
  = let intersperse = case (isListItem, blocks) of
          (True, [Para _, List _ _ _]) -> id
          _                            -> List.intersperse "\n"

        contextFor prev = case prev of
          Just (List _ _ _) -> AfterIndentedList
          _                 -> Normal
    in  (if needsInitialBlanks then "\n\n" else "")
          ++ ( fold
             $ intersperse
             $ mapWithPrev
                 (\prev -> formatMardownBlock formatCode (contextFor prev))
             $ blocks
             )
...
formatMardownBlock formatCode context block = case block of
  ElmDocs terms ->
    ( List.intercalate "\n"
  ...
  Para inlines -> (fold $ fmap (formatMarkdownInline True) $ inlines) ++ "\n"
  ...
  List tight (Bullet _) items ->
    fold $ (if tight then id else List.intersperse "\n") $ fmap
      ( prefix' "  - " "    "
      . formatMarkdown' formatCode True False False
      . toList
      )
      items
  List tight (Numbered _ _) items ->
    fold
      $ (if tight then id else List.intersperse "\n")
      $ fmap (formatListItem formatCode)
      $ zip [1 ..] items

  CodeBlock (CodeAttr lang _info) code ->
    let
      formatted =
        fromMaybe (Text.unpack $ ensureNewline code) $ formatCode $ Text.unpack
          code

      ensureNewline text =
        if Text.last text == '\n' then text else Text.snoc text '\n'

      lang'     = Text.unpack lang

      isElm     = lang' == "elm" || lang' == ""

      canIndent = case context of
        Normal            -> True
  ...
  HtmlBlock text       -> Text.unpack text ++ "\n"

  HRule                -> "---\n"

  ReferencesBlock refs -> fold $ fmap formatRef refs


formatListItem :: (String -> Maybe String) -> (Int, Blocks) -> String
formatListItem formatCode (i, item) =
  let pref = if i < 10 then show i ++ ".  " else show i ++ ". "
  in  prefix' pref "    "
        $ formatMarkdown' formatCode True False False (toList item)
...
formatRef (label, url, title) =
  "["
    ...
    ++ "]: "
    ++ Text.unpack url
    ++ (if Text.unpack title == ""
         then ""
         else " \"" ++ Text.unpack title ++ "\""
       )
    ++ "\n"


prefix' :: String -> String -> String -> String
prefix' preFirst preRest = unlines . prefix preFirst preRest . lines
...
formatMarkdownInline fixSpecialChars inline = case inline of
  Str text ->
  ...
  Code text -> "`" ++ Text.unpack text ++ "`" -- TODO: escape backticks

  Link inlines (Url url) title ->
    let text   = fold $ fmap (formatMarkdownInline False) $ inlines

        title' = Text.unpack title
        url'   = Text.unpack url
    in  if text == url' && title' == ""
          then if fixSpecialChars then "<" ++ url' ++ ">" else url'
          else
            "["
            ++ text
            ++ "]("
  ...
  Entity  text -> Text.unpack text
  RawHtml text -> Text.unpack text
 where
  fix c = case c of
    '\\' -> Text.pack "\\\\"
