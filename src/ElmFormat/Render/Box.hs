...
pleaseReport'' what details =
  "<elm-format-"
    ...
    ++ ": "
    ++ details
    ++ " -- please report this at https://github.com/avh4/elm-format/issues >"


pleaseReport' :: String -> String -> Line
pleaseReport' what details = keyword $ pleaseReport'' what details


pleaseReport :: String -> String -> Box
pleaseReport what details = line $ pleaseReport' what details
...
surround left right b =
  let left'  = punc (left : [])
      right' = punc (right : [])
  in  case b of
        SingleLine b' -> line $ row [left', b', right']
        _             -> stack1 [b |> prefix left', line $ right']
...
formatBinary multiline left ops = case ops of
  [] -> left

  (isLeftPipe, comments, op, next) : rest -> if isLeftPipe
    then ElmStructure.forceableSpaceSepOrIndented
      multiline
      ( ElmStructure.spaceSepOrStack left
      $ concat [Maybe.maybeToList $ formatComments comments, [op]]
      )
      [formatBinary multiline next rest]
    else formatBinary
      multiline
      (ElmStructure.forceableSpaceSepOrIndented
        multiline
        left
        [formatCommented' comments id $ ElmStructure.spaceSepOrPrefix op next]
      )
      rest
...
splitWhere predicate list =
  let merge acc result = ReversedList.push (ReversedList.toList acc) result

      step (acc, result) next = if predicate next
        then (ReversedList.empty, merge (ReversedList.push next acc) result)
        else (ReversedList.push next acc, result)
  in  list
...
declarationType
  :: (a -> BodyEntryType) -> TopLevelStructure a -> DeclarationType
declarationType entryType decl = case decl of
  AST.Declaration.Entry entry -> case entryType $ RA.drop entry of
    BodyNamed name -> DDefinition (Just name)
    BodyUnnamed    -> DDefinition Nothing
    BodyFixity     -> DFixity

  AST.Declaration.DocComment  _                      -> DDocComment

  AST.Declaration.BodyComment AST.CommentTrickOpener -> DStarter

  AST.Declaration.BodyComment AST.CommentTrickCloser -> DCloser

  AST.Declaration.BodyComment _                      -> DComment
...
removeDuplicates input =
 ...
 where
  ...
  step (acc, seen) next =
    case
        foldl stepChildren (ReversedList.empty, seen) next
          |> (\(a, b) -> (ReversedList.toList a, b))
      of
        ([]       , seen') -> (acc, seen')
        (children', seen') -> (ReversedList.push children' acc, seen')

  stepChildren :: Ord a => (Reversed a, Set a) -> a -> (Reversed a, Set a)
  stepChildren (acc, seen) next = if Set.member next seen
    then (acc, seen)
...
sortVars
  :: Bool
  -> Set (AST.Commented AST.Variable.Value)
  -> [[AST.Variable.Ref]]
...
sortVars forceMultiline fromExposing fromDocs =
  let
    ...
    varOrder (AST.Commented _ (AST.Variable.OpValue (SymbolIdentifier name)) _)
      = (1, name)
    varOrder (AST.Commented _ (AST.Variable.Union (UppercaseIdentifier name, _) _) _)
      = (2, name)
    varOrder (AST.Commented _ (AST.Variable.Value (LowercaseIdentifier name)) _)
      = (3, name)
    ...
    listedInExposing = fromExposing |> Set.toList |> List.sortOn varOrder

    varName (AST.Commented _ (AST.Variable.Value (LowercaseIdentifier name)) _)
      = name
    varName (AST.Commented _ (AST.Variable.OpValue (SymbolIdentifier name)) _)
      = name
    varName (AST.Commented _ (AST.Variable.Union (UppercaseIdentifier name, _) _) _)
      = name

    varSetToMap set =
      Set.toList set
        |> fmap
             (\(AST.Commented pre var post) ->
               (varName (AST.Commented pre var post), var)
             )
        |> Map.fromList

    allowedInDocs = varSetToMap fromExposing

    allFromDocs   = Set.fromList $ fmap varName $ concat listedInDocs

    inDocs x = Set.member (varName x) allFromDocs

    remainingFromExposing = listedInExposing |> filter (not . inDocs)
  ...
  in
    if List.null listedInDocs && forceMultiline
      then (fmap (\x -> [x]) remainingFromExposing, commentsFromReorderedVars)
        ...
        ( listedInDocs ++ if List.null remainingFromExposing
          then []
          else [remainingFromExposing]
        , commentsFromReorderedVars
        )
...
formatModuleHeader elmVersion addDefaultHeader modu =
  let
    maybeHeader = if addDefaultHeader
      then Just
        (AST.Module.header modu |> Maybe.fromMaybe AST.Module.defaultHeader)
    ...
    extractDocs block = case block of
      Markdown.ElmDocs vars -> fmap (fmap textToRef) vars
      _                     -> []

    textToRef :: Text -> AST.Variable.Ref
    textToRef text = case Text.unpack text of
      s@(c : _) | Char.isUpper c ->
        AST.Variable.TagRef [] (UppercaseIdentifier s)
      s@(c : _) | Char.isLower c ->
        AST.Variable.VarRef [] (LowercaseIdentifier s)
      '(' : a : ')' : [] -> AST.Variable.OpRef (SymbolIdentifier $ a : [])
      '(' : a : b : ')' : [] ->
        AST.Variable.OpRef (SymbolIdentifier $ a : b : [])
    ...
    AST.KeywordCommented _ _ exportsList = AST.Module.exports
      (maybeHeader |> Maybe.fromMaybe AST.Module.defaultHeader)

    detailedListingToSet
      :: Set (AST.Commented AST.Variable.Value)
      -> AST.Variable.Listing AST.Module.DetailedListing
      -> Set (AST.Commented AST.Variable.Value)
    detailedListingToSet allAvailable (AST.Variable.OpenListing _) =
      allAvailable
    ...
    detailedListingToSet _ (AST.Variable.ExplicitListing (AST.Module.DetailedListing values operators types) _)
      = Set.unions
        [ Map.assocs values
        |> fmap
             (\(name, AST.Commented pre () post) ->
               AST.Commented pre (AST.Variable.Value name) post
             )
        |> Set.fromList
        , Map.assocs operators
        |> fmap
             (\(name, AST.Commented pre () post) ->
               AST.Commented pre (AST.Variable.OpValue name) post
             )
        |> Set.fromList
        , Map.assocs types
        |> fmap
             (\(name, AST.Commented pre (preListing, listing) post) ->
               AST.Commented pre
                             (AST.Variable.Union (name, preListing) listing)
                             post
             )
        |> Set.fromList
        ]
    ...
    detailedListingIsMultiline (AST.Variable.ExplicitListing _ isMultiline) =
      isMultiline
    detailedListingIsMultiline _ = False

    varsToExpose = sortVars (detailedListingIsMultiline exportsList)
                            (detailedListingToSet definedVars exportsList)
                            documentedVars

    extractVarName :: TopLevelStructure Declaration -> [AST.Variable.Value]
    extractVarName decl = case decl of
      AST.Declaration.DocComment  _ -> []
      AST.Declaration.BodyComment _ -> []
      AST.Declaration.Entry (RA.A _ (AST.Declaration.PortAnnotation (AST.Commented _ (LowercaseIdentifier name) _) _ _))
        -> [AST.Variable.Value (LowercaseIdentifier name)]
      AST.Declaration.Entry (RA.A _ (AST.Declaration.Definition (RA.A _ (AST.Pattern.VarPattern (LowercaseIdentifier name))) _ _ _))
        -> [AST.Variable.Value (LowercaseIdentifier name)]
      AST.Declaration.Entry (RA.A _ (AST.Declaration.Definition (RA.A _ (AST.Pattern.Record fields)) _ _ _))
        -> fmap (\(AST.Commented _ f _) -> AST.Variable.Value f) fields
      AST.Declaration.Entry (RA.A _ (AST.Declaration.Datatype (AST.Commented _ (UppercaseIdentifier name, _) _) _))
        -> [ AST.Variable.Union
               (UppercaseIdentifier name, [])
               (AST.Variable.OpenListing (AST.Commented [] () []))
           ]
      AST.Declaration.Entry (RA.A _ (AST.Declaration.TypeAlias _ (AST.Commented _ (UppercaseIdentifier name, _) _) _))
        -> [ AST.Variable.Union (UppercaseIdentifier name, [])
                                AST.Variable.ClosedListing
           ]
      AST.Declaration.Entry (RA.A _ _) -> []

    formatModuleLine' header = case elmVersion of
      Elm_0_16         -> formatModuleLine_0_16 header

      Elm_0_17         -> formatModuleLine elmVersion varsToExpose header

      Elm_0_18         -> formatModuleLine elmVersion varsToExpose header

      Elm_0_18_Upgrade -> formatModuleLine elmVersion varsToExpose header

      Elm_0_19         -> formatModuleLine elmVersion varsToExpose header

      Elm_0_19_Upgrade -> formatModuleLine elmVersion varsToExpose header

    docs =
      fmap (formatDocComment elmVersion (makeImportInfo modu))
        $ RA.drop
        $ AST.Module.docs modu

    imports = formatImports elmVersion modu
  in
    List.intercalate [blankLine] $ concat
      [ maybeToList $ fmap (return . formatModuleLine') maybeHeader
      , maybeToList $ fmap return docs
      , if null imports then [] else [imports]
      ]


formatImports :: ElmVersion -> AST.Module.Module -> [Box]
formatImports elmVersion modu =
  let (comments, imports) = AST.Module.imports modu
  in
    [ formatComments comments |> maybeToList
    , imports |> Map.assocs |> fmap
      (\(name, (pre, method)) -> formatImport elmVersion ((pre, name), method))
    ]
...
formatModuleLine_0_16 header =
  let elmVersion    = Elm_0_16

      formatExports = case AST.Module.exports header of
      ...
      whereClause = case AST.Module.exports header of
        AST.KeywordCommented pre post _ ->
          formatCommented (line . keyword) (AST.Commented pre "where" post)
  in  case
          ( formatCommented (line . formatQualifiedUppercaseIdentifier elmVersion)
            $ AST.Module.name header
          , formatExports
          , whereClause
          )
        of
...
formatModuleLine elmVersion (varsToExpose, extraComments) header =
  let
    tag = case AST.Module.srcTag header of
      AST.Module.Normal        -> line $ keyword "module"

      AST.Module.Port comments -> ElmStructure.spaceSepOrIndented
        (formatTailCommented (line . keyword) ("port", comments))
        [line $ keyword "module"]

      AST.Module.Effect comments -> ElmStructure.spaceSepOrIndented
        (formatTailCommented (line . keyword) ("effect", comments))
        [line $ keyword "module"]

    exports = case varsToExpose of
      [] -> line $ keyword "(..)"
      [oneGroup] ->
        oneGroup
          |> fmap (formatCommented $ formatVarValue elmVersion)
          |> ElmStructure.group' False
                                 "("
                                 ","
                                 (maybeToList (formatComments extraComments))
                                 ")"
                                 False
      _ ->
        varsToExpose
          |> fmap
               ( ElmStructure.group False "" "," "" False
               . fmap (formatCommented $ formatVarValue elmVersion)
               )
          |> ElmStructure.group' False
                                 "("
    ...
    formatSetting (k, v) = formatRecordPair
      elmVersion
      "="
    ...
    formatSettings settings =
      map formatSetting settings |> ElmStructure.group True "{" "," "}" False
    ...
    AST.KeywordCommented preExposing postExposing _ = AST.Module.exports header

    nameClause =
      case
          ( tag
          , formatCommented
              (line . formatQualifiedUppercaseIdentifier elmVersion)
            $ AST.Module.name header
          )
        of
          (SingleLine tag', SingleLine name') ->
            line $ row [tag', space, name']

          (tag', name') -> stack1 [tag', indent $ name']
  in
    ElmStructure.spaceSepOrIndented
      (ElmStructure.spaceSepOrIndented
        ...
        ++ [ formatCommented
               (line . keyword)
               (AST.Commented preExposing "exposing" postExposing)
           ]
        )
      )
      [exports]
...
formatModule elmVersion addDefaultHeader spacing modu =
  let initialComments' = case AST.Module.initialComments modu of
        []       -> []
        comments -> (map formatComment comments) ++ [blankLine, blankLine]

      spaceBeforeBody = case AST.Module.body modu of
        [] -> 0
  ...
  in  stack1 $ concat
        ...
        , maybeToList $ formatModuleBody spacing
                                         elmVersion
                                         (makeImportInfo modu)
                                         (AST.Module.body modu)
        ]
...
makeImportInfo modu =
  let
      ...
      knownModuleContents = Map.fromList $ fmap
        (\(a, b) -> (fmap UppercaseIdentifier a, fmap LowercaseIdentifier b))
        [(["Html", "Attributes"], ["style"])]

      exposed =
          -- currently this only checks for Html.Attributes (needed for Elm 0.19 upgrade)
          let importName = (fmap UppercaseIdentifier ["Html", "Attributes"])
          in  case Map.lookup importName (snd $ AST.Module.imports modu) of
                Nothing -> mempty
                Just (_, importMethod) ->
      ...
      aliases =
          ...
          in  case Map.lookup importName (snd $ AST.Module.imports modu) of
                Nothing                -> mempty
                Just (_, importMethod) -> case AST.Module.alias importMethod of
                  Just (_, (_, alias)) -> Map.singleton importName alias

                  Nothing              -> mempty
  in  ImportInfo exposed aliases
...
formatModuleBody
  ...
  -> ElmVersion
  -> ImportInfo
  -> [TopLevelStructure Declaration]
  -> Maybe Box
formatModuleBody linesBetween elmVersion importInfo body =
  let entryType adecl = case adecl of
        AST.Declaration.Definition pat _ _ _ -> case RA.drop pat of
          AST.Pattern.VarPattern name ->
            BodyNamed $ AST.Variable.VarRef [] name

          AST.Pattern.OpPattern name -> BodyNamed $ AST.Variable.OpRef name
        ...
        AST.Declaration.TypeAnnotation (name, _) _ -> BodyNamed name

        AST.Declaration.PortAnnotation (AST.Commented _ name _) _ _ ->
          BodyNamed $ AST.Variable.VarRef [] name

        AST.Declaration.Fixity _ _ _ _ _    -> BodyFixity

        AST.Declaration.Fixity_0_19 _ _ _ _ -> BodyFixity
  in  formatTopLevelBody linesBetween
                         elmVersion
                         importInfo
                         entryType
                         (formatDeclaration elmVersion importInfo)
                         body
...
formatTopLevelBody linesBetween elmVersion importInfo entryType formatEntry body
  = let
      extraLines n = List.replicate n blankLine

      spacer first second =
        case
            (declarationType entryType first, declarationType entryType second)
          of
            (DStarter, _       ) -> 0
            (_       , DCloser ) -> 0
            (DComment, DComment) -> 0
            ...
            (DComment, DDefinition _) ->
              if linesBetween == 1 then 0 else linesBetween
            ...
            (DDefinition a, DDefinition b) ->
              if a == b then 0 else linesBetween
      ...
      boxes = intersperseMap
        (\a b -> extraLines $ spacer a b)
...
firstOf options value = case options of
  []            -> Nothing
  (next : rest) -> case next value of
...
formatDocComment elmVersion importInfo blocks =
  let
    ...
    parse source = source |> firstOf
      [ fmap DeclarationsCode
      . Result.toMaybe
      . Parse.parseDeclarations elmVersion
      , fmap ExpressionsCode
      . Result.toMaybe
      . Parse.parseExpressions elmVersion
      , fmap ModuleCode . Result.toMaybe . Parse.parseModule elmVersion
      ]

    format :: ElmCodeBlock -> String
    format result = case result of
      ModuleCode modu ->
        formatModule elmVersion False 1 modu |> (Text.unpack . Box.render)
    ...
    content :: String
    content =
      ElmFormat.Render.Markdown.formatMarkdown (fmap format . parse)
        $ fmap cleanBlock blocks

    cleanBlock :: Markdown.Block -> Markdown.Block
    cleanBlock block = case block of
      Markdown.ElmDocs docs -> Markdown.ElmDocs
        $ (fmap . fmap) (Text.replace (Text.pack "(..)") (Text.pack "")) docs
      _ -> block
  in
...
formatDocCommentString docs = case lines docs of
  [] -> line $ row [punc "{-|", space, punc "-}"]
  (first : []) ->
    stack1 [line $ row [punc "{-|", space, literal first], line $ punc "-}"]
  (first : rest) ->
...
formatImport elmVersion (name, method) =
  let
    ...
    formatImportClause
      :: (a -> Maybe Box)
      -> String
      -> (AST.Comments, (AST.Comments, a))
      -> Maybe Box
    formatImportClause format keyw input = case fmap (fmap format) $ input of
      ([], ([], Nothing)) -> Nothing
        ...
        case
          ...
          of
            (SingleLine keyword', SingleLine listing'') ->
              Just $ line $ row [keyword', space, listing'']

            (keyword', listing'') ->
              Just $ stack1 [keyword', indent listing'']

      _ -> Just $ pleaseReport "UNEXPECTED IMPORT"
                               "import clause comments with no clause"
  in
    case
      ( formatHeadCommented
        (line . formatQualifiedUppercaseIdentifier elmVersion)
    ...
    of
      (SingleLine name', Just (SingleLine as'), Just (SingleLine exposing')) ->
        line
          $ row [keyword "import", space, name', space, as', space, exposing']

      (SingleLine name', Just (SingleLine as'), Nothing) ->
        line $ row [keyword "import", space, name', space, as']

      (SingleLine name', Nothing, Just (SingleLine exposing')) ->
        line $ row [keyword "import", space, name', space, exposing']

      (SingleLine name', Nothing, Nothing) ->
        line $ row [keyword "import", space, name']

      (SingleLine name', Just (SingleLine as'), Just exposing') -> stack1
        [ line $ row [keyword "import", space, name', space, as']
        , indent exposing'
        ]

      (SingleLine name', Just as', Just exposing') -> stack1
        [ line $ row [keyword "import", space, name']
        , indent as'
        , indent exposing'
        ]
      ...
      (SingleLine name', Nothing, Just exposing') ->
        stack1 [line $ row [keyword "import", space, name'], indent exposing']

      (name', Just as', Just exposing') -> stack1
        [ line $ keyword "import"
        , indent name'
        , indent $ indent as'
      ...
      (name', Nothing, Just exposing') -> stack1
        [line $ keyword "import", indent name', indent $ indent exposing']

      (name', Just as', Nothing) ->
        stack1 [line $ keyword "import", indent name', indent $ indent as']

      (name', Nothing, Nothing) ->
        stack1 [line $ keyword "import", indent name']


formatListing :: (a -> [Box]) -> AST.Variable.Listing a -> Maybe Box
formatListing format listing = case listing of
  AST.Variable.ClosedListing -> Nothing

  AST.Variable.OpenListing comments ->
    Just $ parens $ formatCommented (line . keyword) $ fmap (const "..")
                                                            comments
...
formatDetailedListing elmVersion listing = concat
...
formatCommentedMap
  :: (k -> v -> a) -> (a -> Box) -> AST.Variable.CommentedMap k v -> [Box]
formatCommentedMap construct format values =
  let format' (k, AST.Commented pre v post) =
          formatCommented format $ AST.Commented pre (construct k v) post
  in  values |> Map.assocs |> map format'


formatVarValue :: ElmVersion -> AST.Variable.Value -> Box
formatVarValue elmVersion aval = case aval of
  AST.Variable.Value val -> line $ formatLowercaseIdentifier elmVersion [] val
  ...
  AST.Variable.Union name listing ->
    case
      ...
      of
        (Just _, _, _, Elm_0_19_Upgrade) -> formatTailCommented
          (\n -> line
            $ row [formatUppercaseIdentifier elmVersion n, keyword "(..)"]
          )
          name

        (Just (SingleLine listing'), SingleLine name', [], _) ->
          line $ row [name', listing']

        (Just (SingleLine listing'), SingleLine name', _, _) ->
          line $ row [name', space, listing']

        (Just listing', name', _, _) -> stack1 [name', indent $ listing']

        (Nothing      , name', _, _) -> name'


formatTopLevelStructure
  :: ElmVersion -> ImportInfo -> (a -> Box) -> TopLevelStructure a -> Box
formatTopLevelStructure elmVersion importInfo formatEntry topLevelStructure =
  case topLevelStructure of
    ...
    AST.Declaration.BodyComment c     -> formatComment c

    AST.Declaration.Entry       entry -> formatEntry (RA.drop entry)


formatDeclaration :: ElmVersion -> ImportInfo -> Declaration -> Box
formatDeclaration elmVersion importInfo decl = case decl of
          ...
          of
            Right args'' ->
              line
                ...
                $ (formatUppercaseIdentifier elmVersion tag)
                : args''
            Left []     -> line $ formatUppercaseIdentifier elmVersion tag
            Left args'' -> stack1
              [ line $ formatUppercaseIdentifier elmVersion tag
              , stack1 args'' |> indent
              ]
    in  case formatOpenCommentedList ctor tags of
          [] -> error "List can't be empty"
          first : rest ->
            case formatCommented (formatNameWithArgs elmVersion) nameWithArgs of
              SingleLine nameWithArgs' -> stack1
                [ line $ row [keyword "type", space, nameWithArgs']
                , first
  ...
  AST.Declaration.Fixity assoc precedenceComments precedence nameComments name
    -> case
      ...
      of
        (SingleLine name', SingleLine precedence') -> line $ row
          [ case assoc of
        ...
        _ -> pleaseReport "TODO" "multiline fixity declaration"

  AST.Declaration.Fixity_0_19 assoc precedence name value ->
    let formatAssoc a = case a of
    ...
    in
      ElmStructure.spaceSepOrIndented
        ...
        , line $ keyword "="
        , formatHeadCommented (line . identifier . formatVarName elmVersion)
                              value
        ]
...
formatNameWithArgs
  :: ElmVersion
  -> (AST.UppercaseIdentifier, [(AST.Comments, AST.LowercaseIdentifier)])
  -> Box
formatNameWithArgs elmVersion (name, args) =
  case
      allSingles $ map
        (formatHeadCommented (line . formatLowercaseIdentifier elmVersion []))
        args
    of
      Right args' ->
        line
          $ row
          $ List.intersperse space
          $ ((formatUppercaseIdentifier elmVersion name) : args')
      Left args' ->
        stack1
...
formatTypeAnnotation
  :: ElmVersion
  -> (AST.Variable.Ref, AST.Comments)
  -> (AST.Comments, AST.Type)
  -> Box
formatTypeAnnotation elmVersion name typ = ElmStructure.definition
  ":"
  False
...
formatPattern elmVersion parensRequired apattern = case RA.drop apattern of
  AST.Pattern.Anything             -> line $ keyword "_"

  AST.Pattern.UnitPattern comments -> formatUnit '(' ')' comments

  AST.Pattern.Literal     lit      -> formatLiteral elmVersion lit
  ...
  AST.Pattern.ConsPattern first rest ->
    let formatRight (preOp, postOp, term, eol) =
            ( False
            , preOp
            , line $ punc "::"
            , formatCommented
  ...
  AST.Pattern.Data ctor [] ->
    line (formatQualifiedUppercaseIdentifier elmVersion ctor)
      |> case (elmVersion, ctor) of
           (Elm_0_16, [_]) -> id
           (Elm_0_16, _  ) -> if parensRequired then parens else id
           _               -> id
  ...
  AST.Pattern.PatternParens pattern ->
    formatCommented (formatPattern elmVersion False) pattern |> parens

  AST.Pattern.Tuple patterns -> ElmStructure.group True "(" "," ")" False
    $ map (formatCommented $ formatPattern elmVersion False) patterns

  AST.Pattern.EmptyListPattern comments -> formatUnit '[' ']' comments

  AST.Pattern.List patterns -> ElmStructure.group True "[" "," "]" False
    $ map (formatCommented $ formatPattern elmVersion False) patterns

  AST.Pattern.EmptyRecordPattern comments -> formatUnit '{' '}' comments

  AST.Pattern.Record fields -> ElmStructure.group True "{" "," "}" False
    $ map
        (formatCommented $ line . formatLowercaseIdentifier elmVersion [])
        fields
    ...
    case
      ...
      of
        (SingleLine pattern', SingleLine name') ->
          line $ row [pattern', space, keyword "as", space, name']

        (pattern', name') ->
          stack1 [pattern', line $ keyword "as", indent name']
...
formatRecordPair
  ...
  -> String
  -> (v -> Box)
  -> (AST.Commented AST.LowercaseIdentifier, AST.Commented v, Bool)
  -> Box
formatRecordPair elmVersion delim formatValue (AST.Commented pre k postK, v, forceMultiline)
  = ElmStructure.equalsPair
      ...
      forceMultiline
      ( formatCommented (line . formatLowercaseIdentifier elmVersion [])
      $ AST.Commented [] k postK
      )
      (formatCommented formatValue v)
    |> (\x -> AST.Commented pre x [])
    |> formatCommented id
...
expressionParens inner outer = case (inner, outer) of
...
formatExpression
  :: ElmVersion -> ImportInfo -> ExpressionContext -> AST.Expression.Expr -> Box
formatExpression elmVersion importInfo context aexpr = case elmVersion of
  Elm_0_19_Upgrade -> formatExpression'
    elmVersion
    importInfo
    context
    (Upgrade_0_19.transform (_exposed importInfo) (_aliases importInfo) aexpr)
  _ -> formatExpression' elmVersion importInfo context aexpr


formatExpression'
  :: ElmVersion -> ImportInfo -> ExpressionContext -> AST.Expression.Expr -> Box
formatExpression' elmVersion importInfo context aexpr = case RA.drop aexpr of
  AST.Expression.Literal lit                -> formatLiteral elmVersion lit

  AST.Expression.VarExpr v                  -> line $ formatVar elmVersion v

  AST.Expression.Range left right multiline -> case elmVersion of
  ...
  AST.Expression.ExplicitList exprs trailing multiline -> formatSequence
    '['
    ','
    (Just ']')
    (formatExpression elmVersion importInfo SyntaxSeparated)
  ...
  AST.Expression.Lambda patterns bodyComments expr multiline ->
    case
        ( multiline
        , allSingles $ map
          ( formatCommented (formatPattern elmVersion True)
          . (\(c, p) -> AST.Commented c p [])
          )
          patterns
        , bodyComments == []
        , formatExpression elmVersion importInfo SyntaxSeparated expr
      ...
      of
        (False, Right patterns', True, SingleLine expr') -> line $ row
          [ punc "\\"
        ...
        (_, Right patterns', _, expr') -> stack1
          [ line
            $ row
                [ punc "\\"
                , row $ List.intersperse space $ patterns'
          ...
          , indent $ stack1 $ (map formatComment bodyComments) ++ [expr']
          ]
        (_, Left [], _, _) -> pleaseReport "UNEXPECTED LAMBDA" "no patterns"
        (_, Left patterns', _, expr') -> stack1
  ...
  AST.Expression.App left args multiline ->
    ElmStructure.application
        ...
        (fmap
          (formatPreCommentedExpression elmVersion importInfo SpaceSeparated)
          args
        )
      |> expressionParens SpaceSeparated context

  AST.Expression.If if' elseifs (elsComments, els) ->
    let opening key cond = case (key, cond) of
          (SingleLine key', SingleLine cond') ->
            line $ row [key', space, cond', space, keyword "then"]
          _ -> stack1 [key, cond |> indent, line $ keyword "then"]

        formatIf (cond, body) = stack1
          [ opening (line $ keyword "if") $ formatCommentedExpression
            elmVersion
            importInfo
            SyntaxSeparated
            cond
          , indent $ formatCommented_
            True
            (formatExpression elmVersion importInfo SyntaxSeparated)
            body
          ]
        ...
        formatElseIf (ifComments, (cond, body)) =
            let key =
                    case
                        (formatHeadCommented id (ifComments, line $ keyword "if"))
                      of
                        SingleLine key' -> line $ row [keyword "else", space, key']
                        key' -> stack1 [line $ keyword "else", key']
            in  stack1
                  [ blankLine
                  , opening key $ formatCommentedExpression elmVersion
                                                            importInfo
                                                            SyntaxSeparated
                                                            cond
                  , indent $ formatCommented_
    ...
    in  formatIf if'
          ...
          |> andThen
               ...
               , indent $ formatCommented_
                 True
                 (formatExpression elmVersion importInfo SyntaxSeparated)
                 (AST.Commented elsComments els [])
               ]
          |> expressionParens AmbiguousEnd context

  AST.Expression.Let defs bodyComments expr ->
    let spacer first _ = case first of
          AST.Expression.LetDefinition _ _ _ _ -> [blankLine]
          _ -> []

        formatDefinition' def = case def of
          ...
          AST.Expression.LetComment comment -> formatComment comment
    in  (line $ keyword "let")
          |> andThen
               (defs |> intersperseMap spacer formatDefinition' |> map indent)
          |> andThen
               [ line $ keyword "in"
               , stack1
  ...
  AST.Expression.Case (subject, multiline) clauses ->
    let
      opening =
        case
            ( multiline
            , formatCommentedExpression elmVersion
                                        importInfo
                                        SyntaxSeparated
                                        subject
            )
          of
            ...
            (_, subject') -> stack1
              [line $ keyword "case", indent subject', line $ keyword "of"]

      clause (pat, expr) =
        case
            ( pat
            , ( formatPattern elmVersion False
              $ (\(AST.Commented _ x _) -> x) pat
              )
          ...
          of
            (_, _, SingleLine pat', body') ->
              stack1 [line $ row [pat', space, keyword "->"], indent body']
            (AST.Commented pre _ [], SingleLine pat', _, body') ->
              stack1
                $  (map formatComment pre)
                ++ [line $ row [pat', space, keyword "->"], indent body']
            (_, _, pat', body') ->
              stack1 $ [pat', line $ keyword "->", indent body']
    in
      opening
      |> andThen
           (clauses |> map clause |> List.intersperse blankLine |> map indent)
      |> expressionParens AmbiguousEnd context -- TODO: not tested

  AST.Expression.Tuple exprs multiline ->
    ElmStructure.group True "(" "," ")" multiline
      $ map
          (formatCommentedExpression elmVersion importInfo SyntaxSeparated)
          exprs
  ...
  AST.Expression.Access expr field ->
    formatExpression elmVersion importInfo SpaceSeparated expr -- TODO: does this need a different context than SpaceSeparated?
                                                               |> addSuffix
      (row $ [punc ".", formatLowercaseIdentifier elmVersion [] field])

  AST.Expression.AccessFunction (AST.LowercaseIdentifier field) ->
    line $ identifier $ "." ++ (formatVarName' elmVersion field)

  AST.Expression.Record base fields trailing multiline -> formatRecordLike
  ...
  AST.Expression.Parens expr -> case expr of
    AST.Commented [] expr' [] ->
  ...
  AST.Expression.Unit comments -> formatUnit '(' ')' comments

  AST.Expression.GLShader src ->
    line $ row [punc "[glsl|", literal $ src, punc "|]"]
...
formatPreCommentedExpression
  ...
  -> ImportInfo
  -> ExpressionContext
  -> AST.PreCommented AST.Expression.Expr
  -> Box
formatPreCommentedExpression elmVersion importInfo context (pre, e) =
  let (pre', e') = case RA.drop e of
        AST.Expression.Parens (AST.Commented pre'' e'' []) ->
          (pre ++ pre'', e'')
        _ -> (pre, e)
  in  formatCommented' pre' (formatExpression elmVersion importInfo context) e'
...
formatRecordLike
  ...
  -> String
  -> (value -> Box)
  -> Maybe (AST.Commented base)
  -> AST.Sequence (AST.Pair key value)
  -> AST.Comments
  -> AST.ForceMultiline
  -> Box
formatRecordLike formatBase formatKey fieldSep formatValue base' fields trailing multiline
  = case (base', fields) of
    (Just base, pairs') -> ElmStructure.extensionGroup'
...
formatSequence left delim right formatA (AST.ForceMultiline multiline) trailing (first : rest)
  = let formatItem delim_ (pre, item) =
    ...
    in  ElmStructure.forceableSpaceSepOrStack
          multiline
          (ElmStructure.forceableRowOrStack multiline
                                            (formatItem left first)
                                            (map (formatItem delim) rest)
          )
          (  maybe [] (flip (:) [] . stack' blankLine) (formatComments trailing)
          ++ (Maybe.maybeToList $ fmap (line . punc . flip (:) []) right)
          )
formatSequence left _ (Just right) _ _ trailing [] =
  formatUnit left right trailing
formatSequence left _ Nothing _ _ trailing [] = formatUnit left ' ' trailing
...
formatBinops_0_19_upgrade
  ...
  -> Box
formatBinops_0_19_upgrade elmVersion importInfo left ops multiline =
  let transform = uncurry removeBangs . uncurry removeMod
  in  formatBinops_common (curry transform)
                          elmVersion
                          importInfo
                          left
                          ops
...
formatBinops_common transform elmVersion importInfo left ops multiline =
  let (left', ops') = transform left ops

      formatPair_ isLast (po, o, pe, e) =
          let isLeftPipe = o == AST.Variable.OpRef (SymbolIdentifier "<|")

              formatContext =
                  if isLeftPipe && isLast then AmbiguousEnd else InfixSeparated
          in  ( isLeftPipe
              , po
              , (line . formatInfixVar elmVersion) o
              , formatCommented'
                pe
                (formatExpression elmVersion importInfo formatContext)
                e
              )
  in  formatBinary
        multiline
...
removeBackticks left ops = case ops of
  [] -> (left, ops)
  (pre, AST.Variable.VarRef v' v, post, e) : rest
    | v == AST.LowercaseIdentifier "andThen" || v == AST.LowercaseIdentifier
      "onError"
    ->
      -- Convert `andThen` to |> andThen
       let e' = noRegion $ AST.Expression.App
       ...
       in  ( left
           , (pre, AST.Variable.OpRef $ SymbolIdentifier "|>", [], e'') : rest'
           )
  ...
  (pre, op, post, e) : rest ->
    ...
    let (e', rest') = removeBackticks e rest
    in  (left, (pre, op, post, e') : rest')
...
removeBangs left ops =
  let
    cmds' post cmds = case RA.drop cmds of
      AST.Expression.ExplicitList [] innerComments _ -> AST.Commented
        post
        (noRegion $ AST.Expression.VarExpr
          (AST.Variable.VarRef [AST.UppercaseIdentifier "Cmd"]
                               (AST.LowercaseIdentifier "none")
          )
        )
        innerComments

      AST.Expression.ExplicitList [(extraPre, (pre', (cmd, eol)))] trailing _
        -> let eolComment = case eol of
                 Nothing -> []
                 Just c  -> [AST.LineComment c]
           in  AST.Commented (post ++ extraPre ++ pre')
                             cmd
                             (eolComment ++ trailing)
    ...
    tuple left' pre post cmds = noRegion $ AST.Expression.Tuple
      [AST.Commented [] left' pre, cmds' post cmds]
      True

    shouldFold opi = case opi of
      ">>" -> FoldIfLeft
      _    -> NeverFold
  in
    binopToFunction (SymbolIdentifier "!")
                    shouldFold
                    tuple
                    left
                    ReversedList.empty
...
removeMod left ops =
  let tuple left' pre post right' = noRegion $ AST.Expression.App
        (noRegion $ AST.Expression.VarExpr $ AST.Variable.OpRef
          (SymbolIdentifier "%")
        )
        [(pre, left'), (post, right')]
        (AST.FAJoinFirst AST.JoinAll)
      ...
      shouldFold opi = case opi of
        ...
        "^"  -> AlwaysFold
        _    -> NeverFold
  in  binopToFunction (SymbolIdentifier "%")
                      shouldFold
                      tuple
                      left
                      ReversedList.empty
...
binopToFunction target shouldFold applyFn left preBang remaining =
  case remaining of
    ...
    (pre, AST.Variable.OpRef sym, post, cmds) : rest | sym == target ->
      let
        left' = case ReversedList.isEmpty preBang of
          True -> left
          False ->
            ( noRegion
            $ AST.Expression.Binops left (ReversedList.toList preBang) False
            )

        (right', rest') = collectRight (collectRightFold . shouldFold)
                                       cmds
                                       ReversedList.empty
                                       rest

        tuple = applyFn left' pre post right'
      in
        binopToFunction target shouldFold applyFn tuple ReversedList.empty rest'

    (pre, op@(AST.Variable.OpRef (SymbolIdentifier opi)), post, e) : rest
      | collectLeftFold $ shouldFold opi -> binopToFunction
        target
        shouldFold
        applyFn
        left
        (ReversedList.push (pre, op, post, e) preBang)
        rest

    (pre, op, post, e) : rest ->
      let (e', rest') = binopToFunction target
                                        shouldFold
                                        applyFn
                                        e
                                        ReversedList.empty
                                        rest
      in  (left, ReversedList.toList preBang ++ (pre, op, post, e') : rest')
...
collectRight shouldFold leftMost collectedLeft rest = case rest of
  (pre, op@(AST.Variable.OpRef (SymbolIdentifier opi)), post, e) : rest'
    | shouldFold opi -> collectRight
      shouldFold
      leftMost
      (ReversedList.push (pre, op, post, e) collectedLeft)
      rest'
  ...
  _ ->
    ...
    ( case ReversedList.isEmpty collectedLeft of
      True  -> leftMost
      False -> noRegion $ AST.Expression.Binops
        leftMost
        (ReversedList.toList collectedLeft)
        False
    , rest
    )
  ...
  case
    ...
    of
      (False, SingleLine left', SingleLine right') ->
        line $ row [punc "[", left', punc "..", right', punc "]"]
      (_, left', right') -> stack1
        [ line $ punc "["
        , indent left'
        , line $ punc ".."
...
nowhere = Region.Position 0 0


noRegion :: a -> RA.Located a
noRegion = RA.at nowhere nowhere
...
formatRange_0_18 elmVersion importInfo context left right =
  case (left, right) of
    (AST.Commented preLeft left' [], AST.Commented preRight right' []) ->
      AST.Expression.App
          ( noRegion
          $ AST.Expression.VarExpr
          $ AST.Variable.VarRef [AST.UppercaseIdentifier "List"]
          $ AST.LowercaseIdentifier "range"
          )
...
formatUnit left right comments = case (left, comments) of
  (_, []) -> line $ punc (left : right : [])

  ('{', (AST.LineComment _) : _) ->
    surround left right $ prefix space $ stack1 $ map formatComment comments

  _ -> surround left right $ case allSingles $ map formatComment comments of
    Right comments' -> line $ row $ List.intersperse space comments'

    Left  comments' -> stack1 comments'


formatComments :: AST.Comments -> Maybe Box
formatComments comments = case fmap formatComment comments of
  []             -> Nothing

  (first : rest) -> Just $ ElmStructure.spaceSepOrStack first rest


formatCommented_ :: Bool -> (a -> Box) -> AST.Commented a -> Box
formatCommented_ forceMultiline format (AST.Commented pre inner post) =
  ElmStructure.forceableSpaceSepOrStack1 forceMultiline $ concat
...
formatCommented = formatCommented_ False


-- TODO: rename to formatPreCommented
formatHeadCommented :: (a -> Box) -> (AST.Comments, a) -> Box
formatHeadCommented format (pre, inner) = formatCommented' pre format inner
...
formatEolCommented format (inner, post) = case (post, format inner) of
  (Nothing, box) -> box
  (Just eol, SingleLine result) ->
    mustBreak $ row [result, space, punc "--", literal eol]
  (Just eol, box) -> stack1 [box, formatComment $ AST.LineComment eol]


formatCommentedStack :: (a -> Box) -> AST.Commented a -> Box
formatCommentedStack format (AST.Commented pre inner post) =
  stack1 $ (map formatComment pre) ++ [format inner] ++ (map formatComment post)
...
formatOpenCommentedList format (AST.OpenCommentedList rest (preLst, lst)) =
  (fmap (formatCommented $ formatEolCommented format) rest)
    ++ [ formatCommented (formatEolCommented format)
           $ AST.Commented preLst lst []
       ]


formatComment :: AST.Comment -> Box
formatComment comment = case comment of
  AST.BlockComment c -> case c of
    []  -> line $ punc "{- -}"
    [l] -> line $ row [punc "{-", space, literal l, space, punc "-}"]
    ls  -> stack1
      [ prefix (row [punc "{-", space]) (stack1 $ map (line . literal) ls)
      , line $ punc "-}"
      ]

  AST.LineComment c       -> mustBreak $ row [punc "--", literal c]

  AST.CommentTrickOpener  -> mustBreak $ punc "{--}"

  AST.CommentTrickCloser  -> mustBreak $ punc "--}"

  AST.CommentTrickBlock c -> mustBreak $ row [punc "{--", literal c, punc "-}"]


formatLiteral :: ElmVersion -> AST.Literal -> Box
formatLiteral elmVersion lit = case lit of
  AST.IntNum i AST.DecimalInt     -> line $ literal $ show i
  AST.IntNum i AST.HexadecimalInt -> line $ literal $ if i <= 0xFF
    then printf "0x%02X" i
    else if i <= 0xFFFF
      then printf "0x%04X" i
      else if i <= 0xFFFFFFFF then printf "0x%08X" i else printf "0x%016X" i
  AST.FloatNum f AST.DecimalFloat  -> line $ literal $ printf "%f" f
  AST.FloatNum f AST.ExponentFloat -> line $ literal $ printf "%e" f
  AST.Chr c                        -> formatString elmVersion SChar [c]
  AST.Str s multi ->
    formatString elmVersion (if multi then SMulti else SString) s
  AST.Boolean b -> line $ literal $ show b
...
formatString elmVersion style s = case style of
  SChar   -> stringBox "\'" id
  SString -> stringBox "\"" id
  SMulti  -> stringBox "\"\"\"" escapeMultiQuote
 where
  stringBox quotes escaper =
    line $ row [punc quotes, literal $ escaper $ concatMap fix s, punc quotes]

  fix c = if (style == SMulti) && c == '\n'
    then [c]
    else if c == '\n'
      then "\\n"
      else if c == '\t'
        then "\\t"
          ...
          then "\\\\"
          else if (style == SString) && c == '\"'
            then "\\\""
            else if (style == SChar) && c == '\''
              then "\\\'"
              else if not $ Char.isPrint c
                then hex c
                else if c == ' '
                  then [c]
                  else
                    if ElmVersion.style_0_19_stringEscape elmVersion
                       == False
                       && c
                       == '\xA0'
                    then
                      ...
                      if Char.isSpace c then hex c else [c]

  hex char = case ElmVersion.style_0_19_stringEscape elmVersion of
    True  -> "\\u{" ++ (printf "%04X" $ Char.ord char) ++ "}"
    False -> "\\x" ++ (printf fmt $ Char.ord char)
    where fmt = if Char.ord char <= 0xFF then "%02X" else "%04X"

  escapeMultiQuote =
    let step okay quotes remaining = case remaining of
          []          -> reverse $ (concat $ replicate quotes "\"\\") ++ okay

          next : rest -> if next == '"'
            then step okay (quotes + 1) rest
            else if quotes >= 3
              then step (next : (concat $ replicate quotes "\"\\") ++ okay)
                        0
                        rest
              else if quotes > 0
...
formatType elmVersion = formatType' elmVersion NotRequired


commaSpace :: Line
commaSpace = row [punc ",", space]


formatTypeConstructor :: ElmVersion -> AST.TypeConstructor -> Box
formatTypeConstructor elmVersion ctor = case ctor of
...
formatType' elmVersion requireParens atype = case RA.drop atype of
  AST.UnitType comments -> formatUnit '(' ')' comments

  AST.FunctionType first rest (AST.ForceMultiline forceMultiline) ->
    let formatRight (preOp, postOp, term, eol) =
            ElmStructure.forceableSpaceSepOrStack1 False $ concat
  ...
  AST.TypeVariable var -> line $ identifier $ formatVarName elmVersion var
  ...
  AST.TupleType types -> ElmStructure.group
    True
    "("
    ","
    ")"
    False
    (map (formatCommented (formatEolCommented $ formatType elmVersion)) types)

  AST.RecordType base fields trailing multiline -> formatRecordLike
    (line . formatLowercaseIdentifier elmVersion [])
    (formatLowercaseIdentifier elmVersion [])
    ":"
...
formatVar elmVersion var = case var of
  AST.Variable.VarRef namespace name ->
    formatLowercaseIdentifier elmVersion namespace name

  AST.Variable.TagRef namespace name -> case namespace of
  ...
  AST.Variable.OpRef name -> formatSymbolIdentifierInParens name
...
formatInfixVar elmVersion var = case var of
  AST.Variable.VarRef _ _ -> row [punc "`", formatVar elmVersion var, punc "`"]
  AST.Variable.TagRef _ _ -> row [punc "`", formatVar elmVersion var, punc "`"]
  AST.Variable.OpRef (SymbolIdentifier name) -> identifier name


formatLowercaseIdentifier
  :: ElmVersion -> [AST.UppercaseIdentifier] -> AST.LowercaseIdentifier -> Line
formatLowercaseIdentifier elmVersion namespace (AST.LowercaseIdentifier name) =
  case (elmVersion, namespace, name) of
    ...
    (Elm_0_18_Upgrade, [AST.UppercaseIdentifier "Basics"], "fst") ->
      identifier "Tuple.first"
    (Elm_0_18_Upgrade, [], "snd") -> identifier "Tuple.second"
    (Elm_0_18_Upgrade, [AST.UppercaseIdentifier "Basics"], "snd") ->
      identifier "Tuple.second"
...
formatQualifiedUppercaseIdentifier
  :: ElmVersion -> [AST.UppercaseIdentifier] -> Line
formatQualifiedUppercaseIdentifier elmVersion names =
  identifier $ List.intercalate "." $ map
    (\(AST.UppercaseIdentifier name) -> formatVarName' elmVersion name)
    names
...
formatVarName' elmVersion name = case elmVersion of
