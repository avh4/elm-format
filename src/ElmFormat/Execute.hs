...
run program operations = do
  let Program init step done = program
  let (initIO, initState)    = init
  initIO
  (result, finalState) <-
    operations |> foldFree step |> flip runStateT initState
  done finalState
  return result
...
forHuman autoYes = Program
  { init = (return (), ())
  , step = \operation -> case operation of
...
forMachine elmVersion autoYes = Program
  { init = Json.init
  , step = \operation -> case operation of
             InFileStore     op -> lift $ FileStore.execute op
