...
usage progName version experimental = fst $ Opt.renderFailure
  (Opt.parserFailure preferences
                     (parser version experimental)
                     Opt.ShowHelpText
                     mempty
  )
  progName
...
preferences = Opt.prefs (mempty <> Opt.showHelpOnError)


parser :: String -> Maybe String -> Opt.ParserInfo Config
parser elmFormatVersion experimental =
  Opt.info (Opt.helper <*> flags) (helpInfo elmFormatVersion experimental)


showHelpText :: String -> Maybe String -> Opt.ParserResult never
showHelpText elmFormatVersion experimental = Opt.Failure $ Opt.parserFailure
  preferences
  (parser elmFormatVersion experimental)
  Opt.ShowHelpText
  mempty
...
helpInfo elmFormatVersion experimental = mconcat
  [ Opt.fullDesc
...
linesToDoc lineList = PP.vcat (map PP.text lineList)

yes :: Opt.Parser Bool
yes = Opt.switch
  $ mconcat [Opt.long "yes", Opt.help "Reply 'yes' to all automated prompts."]

validate :: Opt.Parser Bool
validate = Opt.switch $ mconcat
  [ Opt.long "validate"
  , Opt.help "Check if files are formatted without changing them."
  ]

dependencies :: Opt.Parser Bool
dependencies = Opt.switch $ mconcat
...
output = Opt.optional $ Opt.strOption $ mconcat
  [ Opt.long "output"
  , Opt.metavar "FILE"
  , Opt.help
...
input = Opt.strArgument $ Opt.metavar "INPUT"

stdin :: Opt.Parser Bool
stdin = Opt.switch
  $ mconcat [Opt.long "stdin", Opt.help "Read from stdin, output to stdout."]


elmVersion :: Opt.Parser (Maybe ElmVersion)
elmVersion =
  Opt.optional $ Opt.option (Opt.eitherReader ElmVersion.parse) $ mconcat
...
upgrade = Opt.switch $ mconcat
  [Opt.long "upgrade", Opt.help "Upgrade older Elm files to Elm 0.19 syntax"]


json :: Opt.Parser Bool
json = Opt.switch $ mconcat
  [ Opt.long "json"
