...
containerContinue c = case containerType c of
  ...
  FencedCode { startColumn = col } -> scanSpacesToColumn col
  RawHtmlBlock                     -> nfb scanBlankline
  li@ListItem{} ->
    scanBlankline
      <|> (do
            scanSpacesToColumn (markerColumn li + 1)
            _ <- upToCountChars (padding li - 1) (== ' ')
            return ()
          )
  Reference{} -> nfb scanBlankline >> nfb (scanNonindentSpace *> scanReference)
  _           -> return ()
...
containerStart _lastLineIsText =
  scanNonindentSpace
    *> ((BlockQuote <$ scanBlockquoteStart) <|> parseListMarker)
  ...
  scanNonindentSpace
    *> (   parseCodeFence
       <|> (  guard (not lastLineIsText)
           *> (IndentedCode <$ char ' ' <* nfb scanBlankline)
           )
       <|> (  guard (not lastLineIsText)
           *> (RawHtmlBlock <$ parseHtmlBlockStart)
           )
...
closeStack = do
  ...
  if null rest then return top else closeContainer >> closeStack
...
closeContainer = do
  ...
  case top of
    (Container Reference{} cs'') ->
      case
          parse pReference (T.strip $ joinLines $ map extractText $ toList cs'')
        of
          ...
          Left _ -> -- pass over in silence if ref doesn't parse?
            ...
            []       -> return ()
    (Container li@ListItem{} cs'') -> case rest of
           -- move final BlankLine outside of list item
      (Container ct' cs' : rs) -> case viewr cs'' of
        (zs :> b@(L _ BlankLine{})) -> put $ ContainerStack
          (if Seq.null zs
            then Container ct' (cs' |> C (Container li zs))
            else Container ct' (cs' |> C (Container li zs) |> b)
          )
          rs
...
addLeaf lineNum lf = do
  ContainerStack top rest <- get
  case (top, lf) of
    (Container ct@(ListItem{}) cs, BlankLine{}) -> case viewr cs of
...
processDocument (Container ct cs, refmap) = case ct of
...
processElts refmap (L _lineNumber lf : rest) = case lf of
    -- Special handling of @docs lines in Elm:
  TextLine t | Just terms1 <- T.stripPrefix "@docs" t ->
    let docs = terms1 : map (cleanDoc . extractText) docLines
    in  singleton
            (ElmDocs $ filter ((/=) []) $ fmap
              (filter ((/=) "") . fmap T.strip . T.splitOn ",")
              docs
            )
   ...
   where
    ...
    isDocLine _                  = False
    cleanDoc lin = case T.stripPrefix "@docs" lin of
      Nothing       -> lin
      Just stripped -> stripped

  -- Gobble text lines and make them into a Para:
  TextLine t ->
    singleton (Para $ parseInlines refmap txt) <> processElts refmap rest'
   where
    txt =
      T.stripEnd $ joinLines $ map T.stripStart $ t : map extractText textlines
  ...
  -- Headers:
  ATXHeader lvl t ->
    singleton (Header lvl $ parseInlines refmap t) <> processElts refmap rest
  SetextHeader lvl t ->
    singleton (Header lvl $ parseInlines refmap t) <> processElts refmap rest
...
processElts refmap (C (Container ct cs) : rest) = case ct of
  ...
  ListItem { listType = listType' } ->
   ...
   where
    ...
    takeListItems (C c@(Container ListItem { listType = lt' } _) : zs)
      | listTypesMatch lt' listType' = C c : takeListItems zs
    takeListItems (lf@(L _ (BlankLine _)) : c@(C (Container ListItem { listType = lt' } _)) : zs)
  ...
  FencedCode _ _ info' ->
    singleton (CodeBlock attr txt) <> processElts refmap rest
   where
  ...
  IndentedCode ->
    singleton (CodeBlock (CodeAttr "" "") txt) <> processElts refmap rest'
   where
    txt = joinLines $ stripTrailingEmpties $ concatMap extractCode cbs
    ...
    extractCode (C (Container IndentedCode cs')) = map extractText $ toList cs'
    extractCode _ = []

    (cbs, rest') = span isIndentedCodeOrBlank (C (Container ct cs) : rest)

    isIndentedCodeOrBlank (L _ BlankLine{}             ) = True
    isIndentedCodeOrBlank (C (Container IndentedCode _)) = True
  ...
  -- so we just skip.
  Reference{} -> processElts' [] (C (Container ct cs) : rest)
   where
    refs cs' = fmap (extractRef . extractText) (toList cs')

    extractRef t = case parse pReference (T.strip t) of
      Right (lab, lnk, tit) -> (lab, lnk, tit)
      Left  _               -> ("??", "??", "??")
...
processLine (lineNumber, txt) = do
  ...
  let lastLineIsText = numUnmatched == 0 && case viewr cs of
    ...
    _ ->
      case tryNewContainers lastLineIsText (T.length txt - T.length t') t' of
       ...
       -- some unmatched containers:
        ...
        ([], SetextHeader lev _) | numUnmatched == 0 -> case viewr cs of
          (cs' :> L _ (TextLine t)) -> -- replace last text line with setext header
                                       put $ ContainerStack
            (Container ct (cs' |> L lineNumber (SetextHeader lev t)))
            rest
...
tryOpenContainers cs t = case parse (scanners $ map containerContinue cs) t of
 ...
 where
  ...
  scanners (p : ps) =
    (p *> scanners ps) <|> ((,) <$> takeText <*> pure (length (p : ps)))
...
tryNewContainers lastLineIsText offset t = case parse newContainers t of
  Right (cs, t') -> (cs, t')
  ...
  newContainers = do
    ...
    if null verbatimContainers
      then (,) <$> pure regContainers <*> leaf lastLineIsText
        ...
        (,) <$> pure (regContainers ++ verbatimContainers) <*> textLineOrBlank
...
parseCodeFence = do
  ...
  endOfInput
  return $ FencedCode { startColumn = col, fence = cs, info = rawattr }
...
parseListNumber = do
  ...
  wrap <- PeriodFollowing <$ skip (== '.') <|> ParenFollowing <$ skip (== ')')
