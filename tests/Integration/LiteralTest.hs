module Integration.LiteralTest
  ( tests
  )
where
...
makeTest (original, formatted) = testCase original $ assertEqual
...
format source =
  source |> Text.pack |> Parse.parseLiteral |> Parse.toEither |> fmap
    (Render.renderLiteral ElmVersion.Elm_0_18)
