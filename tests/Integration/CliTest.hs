...
import           ElmFormat.TestWorld                      ( TestWorld
                                                          , run
                                                          , expectExit
                                                          , goldenExitStdout
                                                          , expectFileContents
...
tests = testGroup
  "CLI"
  [ testCase "usage" $ world |> run "elm-format" ["--help"] |> expectExit 0
  , world |> run "elm-format-xxx" [] |> TestWorld.goldenStdout
    "usage instructions"
    "tests/usage.stdout"
  , testCase "simple file"
  $  world
  ...
  |> TestWorld.goldenStderr "using --stdin writes errors to stderr"
                            "tests/stdin-error.stderr"
  , testGroup
    "auto-detects Elm version"
    [ testCase "for Elm 0.19 applications"
    $  world
    |> TestWorld.uploadFile
         "test.elm"
         "module Main exposing (f)\n\n\nf =\n    '\\u{2000}'\n"
    |> TestWorld.uploadFile "elm.json" "{\"elm-version\": \"0.19.0\"}"
    |> run "elm-format" ["test.elm", "--validate"]
    |> expectExit 0
    , testCase "for Elm 0.19 packages"
    $  world
    |> TestWorld.uploadFile
         "test.elm"
         "module Main exposing (f)\n\n\nf =\n    '\\u{2000}'\n"
    |> TestWorld.uploadFile "elm.json"
                            "{\"elm-version\": \"0.19.0 <= v < 0.20.0\"}"
    |> run "elm-format" ["test.elm", "--validate"]
    |> expectExit 0
    , testCase "for Elm 0.18"
    $  world
    |> TestWorld.uploadFile
         "test.elm"
         "module Main exposing (f)\n\n\nf =\n    '\\x2000'\n"
    |> TestWorld.uploadFile "elm-package.json"
                            "{\"elm-version\": \"0.18.0 <= v < 0.19.0\"}"
    |> run "elm-format" ["test.elm", "--validate"]
    |> expectExit 0
    , testCase "default to Elm 0.19"
    $  world
    |> TestWorld.uploadFile
         "test.elm"
         "module Main exposing (f)\n\n\nf =\n    '\\u{2000}'\n"
  ...
  , testGroup
    "ways to run"
    [ goldenExitStdout "elm-format --help" 0 "tests/usage.stdout" $ world |> run
      "elm-format-xxx"
      ["--help"]
    , goldenExitStdout "elm-format -h" 0 "tests/usage.stdout" $ world |> run
      "elm-format-xxx"
      ["-h"]
    , goldenExitStdout "elm-format (no args)" 1 "tests/usage.stdout"
    $  world
    |> run "elm-format-xxx" []
    , testCase "elm-format INPUT --yes does change the file"
    $  world
    ...
    , testCase "elm-format INPUT --validate does not change things"
    $  world
    ...
    , testCase "elm-format INPUT --validate with unformatted file exits 1"
    $  world
    |> TestWorld.uploadFile "unformatted.elm" unformatted_elm
    |> run "elm-format" ["unformatted.elm", "--elm-version=0.19", "--validate"]
    |> expectExit 1
    , testCase "elm-format INPUT --validate with formatted file exits 0"
    $  world
...
unformatted_elm = unlines ["module MyMain exposing (x)", "x = ()"]

formatted_elm :: String
formatted_elm = unlines ["module MyMain exposing (x)", "", "", "x =", "    ()"]
...
world =
  TestWorld.init
    ...
    |> TestWorld.installProgram "elm-format-xxx"
                                (ElmFormat.main'' "x.x.x" Nothing)
