...
fullStdout state = stdout state |> reverse |> concat


fullStderr :: TestWorldState -> String
fullStderr state = stderr state |> reverse |> concat
...
instance World (State.State TestWorldState) where
  ...
  doesDirectoryExist _path = return False
  ...
  writeFile path content = do
    state <- State.get
    State.put
      $ state { filesystem = Dict.insert path content (filesystem state) }

  writeUtf8File path content = writeFile path (StrictText.unpack content)
  ...
  writeStdout text = putStr (StrictText.unpack text)

  putStrStderr string = do
    state <- State.get
    State.put $ state { stderr = string : stderr state }

  putStrLnStderr string = do
    state <- State.get
...
testWorld files = TestWorldState { filesystem   = Dict.fromList files
                                 , queuedStdin  = ""
                                 , stdout       = []
                                 , stderr       = []
                                 , programs     = mempty
...
queueStdin newStdin state = state { queuedStdin = newStdin }


assertOutput :: [(String, String)] -> TestWorldState -> Assertion
assertOutput expectedFiles context = assertBool
  (  "Expected filesystem to contain: "
  ++ show expectedFiles
  ++ "\nActual: "
  ++ show (filesystem context)
  )
  (all (\(k, v) -> Dict.lookup k (filesystem context) == Just v) expectedFiles)


goldenStdout :: String -> FilePath -> TestWorldState -> TestTree
goldenStdout = goldenOutputStream fullStdout


goldenStderr :: String -> FilePath -> TestWorldState -> TestTree
goldenStderr = goldenOutputStream fullStderr


goldenOutputStream
  :: (TestWorldState -> String)
  -> String
  -> FilePath
  -> TestWorldState
  -> TestTree
goldenOutputStream getStream testName goldenFile state = goldenVsStringDiff
  testName
...
goldenExitStdout testName expectedExitCode goldenFile state = testGroup
  testName
...
downloadFile name world = Dict.lookup name (filesystem world)


installProgram
  :: String -> ([String] -> State.State TestWorld ()) -> TestWorld -> TestWorld
...
run name args testWorld = case Dict.lookup name (programs testWorld) of
  Nothing -> testWorld
  ...
  Just handler ->
    let ((), newTestWorld) = State.runState (handler args) testWorld
    in  newTestWorld


expectExit :: Int -> TestWorld -> Assertion
expectExit expectedExitCode testWorld = case lastExitCode testWorld of
  Nothing -> fail
    (  "Expected last exit code to be: "
    ++ show expectedExitCode
    ++ ", but no program was executed"
    )
...
expectFileContents filename expectedContent testWorld =
  case downloadFile filename testWorld of
    Nothing -> fail
      (  "Expected file "
      ++ show filename
      ++ " to have contents, but it did not exist"
      )

    Just actualContent -> assertEqual ("contents of " ++ show filename)
                                      expectedContent
                                      actualContent
