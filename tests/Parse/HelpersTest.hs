...
example parser name input expected =
  testCase name $ assertParse parser input expected
...
tests = testGroup
  ...
  [ testGroup
      ...
      , example (parens'' x) "comments"       "({-A-}x{-B-})"
        $ Right [Commented [BlockComment ["A"]] "x" [BlockComment ["B"]]]
      , example (parens'' x) "multiple terms" "(a,b,c)"
        $ Right [Commented [] "a" [], Commented [] "b" [], Commented [] "c" []]
      , example (parens'' x) "whitespace"     "( a , b , c )"
        $ Right [Commented [] "a" [], Commented [] "b" [], Commented [] "c" []]
