...
import           Parse.TestHelpers                        ( assertParse
                                                          , assertParseFailure
                                                          )
...
example name input expected = testCase name $ assertParse
  (fmap (unpack . render . formatLiteral ElmVersion.Elm_0_18) literal)
  input
  expected
...
tests = testGroup
  ...
  , testGroup
    ...
    , testCase "exponent must have digits" $ assertParseFailure literal "e3"
    ...
    , testCase "must have digits" $ assertParseFailure literal "."
    , testCase "must start with a digit" $ assertParseFailure literal ".1"
