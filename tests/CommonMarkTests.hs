...
step (ParseState path input output state siblings children example) line =
  if state == None && fmap fst (Text.uncons line) == Just '#'
    then
    ...
    else if Text.unpack line == "```````````````````````````````` example"
      then
        -- validate input, output is [], state is None
           ParseState { path     = path
                      , input    = []
                      , output   = []
        ...
        then
          ...
          , state    = None
          , siblings = siblings
          , children = makeTest example
                                (concat $ reverse input)
                                (unlines $ reverse input)
                                (unlines $ reverse output)
                         : children
          , example  = example + 1
          }
        else if state == CollectingInput && Text.unpack line == "."
          then
            -- validate output == []
               ParseState { path     = path
                          , input    = input
            ...
            CollectingInput -> ParseState { path     = path
            ...
            CollectingOutput -> ParseState { path     = path
                                           , input    = input
...
makeTest i name input output =
  let source =
          Strict.map (\c -> if c == '→' then '\t' else c) $ Strict.pack $ input

      formatted = ElmFormat.Render.Markdown.formatMarkdown
        (const Nothing)
        (Parse.Markdown.parse $ Strict.unpack source)

      -- specOutput = Strict.map (\c -> if c == '→' then '\t' else c) $ Strict.pack output

      description =
          "formatted markdown should render the same as the original\n\n"
            ++ Strict.unpack source
            ++ "\n"
            ++ formatted
  in  testCase ("Example " ++ show i ++ ": " ++ name) $ assertEqual
        description
...
construct = do
  ...
  return $ testGroup "CommonMark" $ done $ foldl step init (Text.lines spec)
