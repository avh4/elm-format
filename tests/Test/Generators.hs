...
capitalLetter = elements "ABCDEFGHIJKLMNOPQRSTUVWXYZÀΩЉԱႠḀℇ𐐅𝐴𝞨"
    -- Should also be capital, but Data.Char.isUpper does not agree: Ꭳ


capitalAscii :: Gen Char
capitalAscii = elements "ABCDEFGHIJKLMNOPQRSTUVWXYZ"


lowerLetter :: Gen Char
lowerLetter = elements "abcdefghijklmnopqrstuvwxyz"


number :: Gen Char
number = elements "0123456789"
...
nowhere = Reporting.Region.Position 0 0


located :: a -> Reporting.Annotation.Located a
located = Reporting.Annotation.at nowhere nowhere


instance Arbitrary Reporting.Region.Region where
  arbitrary = return $ Reporting.Region.Region nowhere nowhere
...
listing = return $ AST.Variable.OpenListing (Commented [] () [])
...
instance Arbitrary AST.Module.Module where
  arbitrary = do
    ...
    listing    <- listing
    moduleType <- fmap
      (\x -> if x then AST.Module.Port [] else AST.Module.Normal)
      arbitrary
    return $ AST.Module.Module
      []
      (Just $ AST.Module.Header moduleType
                                (Commented [] name [])
                                Nothing
      ...
      (located Nothing)
      ([], empty)
      [ AST.Declaration.Entry $ located $ AST.Declaration.Definition
          (located $ AST.Pattern.Anything)
          []
          []
          (located $ AST.Expression.TupleFunction 2)
      ]
