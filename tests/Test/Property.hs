...
assertStringToString source =
  let source' = Text.pack source

      result =
          Parse.parse ElmVersion.Elm_0_19 source' |> Parse.toEither |> fmap
            (Render.render ElmVersion.Elm_0_19)
  in  assertEqual "" (Right source') result
...
astToAst ast =
  let result =
          ast
            ...
            |> Parse.toEither
  in  assertEqual "" (Right $ stripRegion ast) (fmap stripRegion result)


simpleAst =
  case
      Parse.toEither $ Parse.parse ElmVersion.Elm_0_19 $ Text.pack
...
reportFailedAst ast =
  let rendering = Render.render ElmVersion.Elm_0_19 ast |> Text.unpack
      result =
...
withCounterexample fn prop = (\s -> counterexample (fn s) $ prop s)


propertyTests :: TestTree
propertyTests = testGroup
  "example test group"
  [ testCase "simple AST round trip" $ astToAst simpleAst
  ...
  , testGroup
    ...
    $  forAll Test.ElmSourceGenerators.elmModule
    $  withCounterexample id
    $  Text.pack
    >> Parse.parse ElmVersion.Elm_0_19
    >> Parse.toMaybe
    >> Maybe.isJust
  ...
  , testCase "simple round trip with comments"
    $ assertStringToString
        "module Main exposing (foo)\n\n\nfoo =\n    ( {- A -} 3 {- B -}, {- C -} 4 {- D -} )\n"
  , testCase "simple round trip with comments"
    $ assertStringToString
        "module Main exposing (commentedLiterals)\n\n\ncommentedLiterals =\n    ( {- int -} 1, {- float -} 0.1, {- char -} \'c\', {- string -} \"str\", {- boolean -} True )\n"
  ]
