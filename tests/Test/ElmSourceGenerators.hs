...
wsOptional = oneof [wsRequired, return ""]


withWhitespace :: Gen String -> [String] -> Gen String
withWhitespace ws components =
  let step :: String -> Gen String -> Gen String
      step a b = do
        b'  <- b
        ws' <- ws
        return $ a ++ ws' ++ b'
  in  foldr step (return "") components
...
expression =
  let unit    = withWhitespace wsOptional ["(", ")"]
      tuple   = withWhitespace wsOptional $ words "( a , b , c )"
      literal = elements ["1", "2.0", "'x'", "\"ABC\"", "\"\"\"z\nz\n\"\"\""]
      app     = do
        e0 <- expression
        e1 <- expression
        e2 <- expression
        return $ unwords [e0, e1, e2]
  in  oneof [unit, literal, app, tuple]
