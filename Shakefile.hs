...
main = do
  ...
  shakeArgs shakeOptions { shakeChange  = ChangeModtimeAndDigest
                         , shakeColor   = True
                         , shakeVersion = shakefilesHash
                         }
    $ do
        StdoutTrim stackLocalInstallRoot <- liftIO
          $ cmd "stack path --local-install-root"
        ...
        "generated/Build_elm_format.hs" %> \out -> do
          alwaysRerun
          (StdoutTrim gitDescribe) <- cmd
            "git"
            ["describe", "--abbrev=8", "--always"]
        ...
        "_build/stack-test.ok" %> \out -> do
          testFiles <- getDirectoryFiles
            ""
            ["tests//*.hs", "tests//*.stdout", "tests//*.stderr"]
          need testFiles
          sourceFiles <- getDirectoryFiles "" sourceFilesPattern
          need sourceFiles
          need generatedSourceFiles
        ...
        forM_ elmVersions $ \elmVersion -> do
          ("_build/tests/test-files/good/Elm-" ++ elmVersion ++ ".ok")
            %> \out -> do
                 elmFiles <- getDirectoryFiles
                   ""
                   ["tests/test-files/good/Elm-" ++ elmVersion ++ "//*.elm"]
                 let oks =
                       [ "_build" </> f -<.> "elm_matches" | f <- elmFiles ]
                 need oks
                 writeFile' out (unlines elmFiles)

          ("_build/tests/test-files/*/Elm-" ++ elmVersion ++ "//*.elm_formatted"
            )
            %> \out -> do
          ...
          ("_build/tests/test-files/*/Elm-" ++ elmVersion ++ "//*.elm_upgraded")
            %> \out -> do
                 let source = dropDirectory1 $ out -<.> "elm"
                 need [elmFormat, source]
                 cmd_ elmFormat
          ...
          ("_build/tests/test-files/*/Elm-" ++ elmVersion ++ "//*.elm_stderr")
            %> \out -> do
                 let source = dropDirectory1 $ out -<.> "elm"
                 need [elmFormat, source]
                 (Stderr stderr, Exit _) <- cmd
                   (FileStdin source)
                   BinaryPipes
                   elmFormat
                   "--stdin"
          ...
          ("_build/tests/test-files/transform/Elm-" ++ elmVersion ++ ".ok")
            %> \out -> do
                 elmFiles <- getDirectoryFiles
                   ""
                 ...
                 let oks =
                       [ "_build" </> f -<.> "elm_transform_matches"
                       | f <- elmFiles
                       , (takeExtension $ dropExtension f) /= ".formatted"
                       ]
                 need oks
                 writeFile' out (unlines oks)

          ("_build/tests/test-files/upgrade/Elm-" ++ elmVersion ++ ".ok")
            %> \out -> do
                 elmFiles <- getDirectoryFiles
                   ""
                   ["tests/test-files/upgrade/Elm-" ++ elmVersion ++ "//*.elm"]
                 let oks =
                       [ "_build" </> f -<.> "elm_upgrade_matches"
                       | f <- elmFiles
                       , (takeExtension $ dropExtension f) /= ".formatted"
                       ]
                 need oks
          ...
          ("_build/tests/test-files/bad/Elm-" ++ elmVersion ++ ".ok")
            %> \out -> do
                 elmFiles <- getDirectoryFiles
                   ""
                   ["tests/test-files/bad/Elm-" ++ elmVersion ++ "//*.elm"]
                 let oks =
                       [ "_build" </> f -<.> "elm_bad_matches" | f <- elmFiles ]
                 need oks
                 writeFile' out (unlines elmFiles)
        ...
        "_build/tests/test-files/good-json.ok" %> \out -> do
          jsonFiles <- getDirectoryFiles "" ["tests/test-files/good//*.json"]
        ...
        "_build/tests//*.json_formatted" %> \out -> do
          let source = dropDirectory1 $ out -<.> "elm"
          need [elmFormat, source]
          (Stdout rawJson) <- cmd (FileStdin source)
                                  elmFormat
                                  "--elm-version=0.19"
                                  "--stdin"
                                  "--json"
