...
import Text.Parsec hiding (State, newline, spaces)
...
spaceySepBy1 sep parser =
  let -- step :: PostCommented (WithEol a) -> [Commented (WithEol a)] -> Comments -> IParser (ExposedCommentedList a)
...
toSet merge values =
  let merge' (C (pre1, post1) a) (C (pre2, post2) b) =
        C (pre1 ++ pre2, post1 ++ post2) (merge a b)
   in foldl (\m (C (pre, post) (k, v)) -> insertWith merge' k (C (pre, post) v) m) empty values
...
separated sep expr' =
  let subparser =
   ...
   in (\(f, multiline) -> f $ multilineToBool multiline) <$> trackNewline subparser
