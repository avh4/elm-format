module Parse.ExpressionSpec where
...
spec :: Spec
spec = describe "Parse.Expression" $ do
    describe "Unit" $ do
    ...
    describe "unary operators" $ do
        describe "negative" $ do
            ...
            it "must not have whitespace" $
                assertParseFailure (expr Elm_0_19) "- True"
            it "must not have comment" $
                assertParseFailure (expr Elm_0_19) "-{- -}True"
    ...
    describe "tuple constructor" $ do
        ...
        it "does not allow whitespace (1)" $ assertParseFailure (expr Elm_0_19) "( ,,)"
        it "does not allow whitespace (2)" $ assertParseFailure (expr Elm_0_19) "(, ,)"
        it "does not allow whitespace (3)" $ assertParseFailure (expr Elm_0_19) "(,, )"
        it "does not allow comments (1)" $ assertParseFailure (expr Elm_0_19) "({-A-},,)"
        it "does not allow comments (2)" $ assertParseFailure (expr Elm_0_19) "(,{-A-},)"
        it "does not allow comments (3)" $ assertParseFailure (expr Elm_0_19) "(,,{-A-})"
    ...
    describe "record access" $ do
        ...
        it "does not allow symbolic field names" $
            assertParseFailure (expr Elm_0_19) "x.+"
        it "does not allow symbolic field names" $
            assertParseFailure (expr Elm_0_19) "x.(+)"

    describe "record access fuction" $ do
    ...
    describe "lambda" $ do
        ...
        it "arrow must not contain whitespace" $
    ...
    describe "let statement" $ do
        ...
        it "must have at least one definition" $
            assertParseFailure (expr Elm_0_19) "let in z"
        describe "declarations must start at the same column" $ do
            it "(1)" $ assertParseFailure (expr Elm_0_19) "let a=b\n   c=d\nin z"
            it "(2)" $ assertParseFailure (expr Elm_0_19) "let a=b\n     c=d\nin z"
            it "(3)" $ assertParseFailure (expr Elm_0_19) "let  a=b\n   c=d\nin z"
    ...
    describe "case statement" $ do
        ...
        it "should not consume trailing whitespace" $
            assertParse (expr Elm_0_19>> string "\nX") "case 9 of\n 1->10\n _->20\nX" "\nX"
        describe "clauses must start at the same column" $ do
            it "(1)" $ assertParseFailure (expr Elm_0_19) "case 9 of\n 1->10\n_->20"
            it "(2)" $ assertParseFailure (expr Elm_0_19) "case 9 of\n 1->10\n  _->20"
            it "(3)" $ assertParseFailure (expr Elm_0_19) "case 9 of\n  1->10\n _->20"
