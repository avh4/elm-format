...
import           Text.Parsec.Indent                       ( block
                                                          , withPos
                                                          , checkIndent
                                                          )
...
listTerm elmVersion = shader' <|> try (braces range) <|> commaSeparated
 where
  range = do
    lo                 <- expr elmVersion
    (loPost, _, hiPre) <- padded (string "..")
    hi                 <- expr elmVersion
    return $ \loPre hiPost multiline -> E.Range (Commented loPre lo loPost)
...
parensTerm elmVersion = choice
 ...
 where
  opFn    = E.VarExpr <$> anyOp elmVersion
  ...
  parened = do
    expressions <- commaSep1 ((\e a b -> Commented a e b) <$> expr elmVersion)
    return $ \pre post multiline -> case expressions pre post of
      [single]     -> E.Parens single

      expressions' -> E.Tuple expressions' multiline

  unit = return $ \pre post _ -> E.Unit (pre ++ post)


recordTerm :: ElmVersion -> IParser E.Expr
recordTerm elmVersion = addLocation $ brackets' $ checkMultiline $ do
  base <- optionMaybe $ try (commented (lowVar elmVersion) <* string "|")
  (fields, trailing) <- sectionedGroup
    (pair (lowVar elmVersion) lenientEquals (expr elmVersion))
...
term elmVersion =
  addLocation
    ...
    <|> accessible
          elmVersion
          (   addLocation (varTerm elmVersion)
...
appExpr elmVersion = expecting "an expression" $ do
  start                     <- getMyPosition
  ...
  return $ case ts of
    ...
    _ ->
      ...
      in  A.at start end $ E.App t (fmap fst ts) multiline
...
binaryExpr elmVersion = Binop.binops (appExpr elmVersion)
                                     lastExpr
                                     (anyOp elmVersion)
 where
  lastExpr =
...
ifExpr elmVersion =
  let elseKeyword =
          (reserved elmVersion "else" <?> "an 'else' branch") >> whitespace
  in  do
...
lambdaExpr elmVersion =
  ...
  in  addLocation $ do
        ((args, preArrowComments, bodyComments, body), multiline) <-
          trackNewline subparser
        return
          $ E.Lambda args (preArrowComments ++ bodyComments) body
          $ multilineToBool multiline


caseExpr :: ElmVersion -> IParser E.Expr'
caseExpr elmVersion = do
  try (reserved elmVersion "case")
  (e, multilineSubject) <-
    trackNewline $ (\(pre, e, post) -> Commented pre e post) <$> padded
      (expr elmVersion)
 ...
 where
  case_ preComments = do
    (patternComments, p, (preArrowComments, _, bodyComments)) <- try
      (   (,,)
      <$> whitespace
...
letExpr elmVersion = do
  try (reserved elmVersion "let")
  commentsAfterLet <- map E.LetComment <$> whitespace
  defs             <- block $ do
    def <-
      typeAnnotation elmVersion E.LetAnnotation
        <|> definition elmVersion E.LetDefinition
...
typeAnnotation
  :: ElmVersion -> ((Var.Ref, Comments) -> (Comments, Type) -> a) -> IParser a
typeAnnotation elmVersion fn =
  (\(v, pre, post) e -> fn (v, pre) (post, e))
    <$> try start
    <*> Type.expr elmVersion
 where
  start = do
    v <- (Var.VarRef [] <$> lowVar elmVersion) <|> (Var.OpRef <$> symOpInParens)
...
definition
  :: ElmVersion
  -> (P.Pattern -> [(Comments, P.Pattern)] -> Comments -> E.Expr -> a)
  -> IParser a
definition elmVersion fn = withPos $ do
...
defStart elmVersion =
  choice
      [ do
        pattern <- try $ Pattern.term elmVersion
        func pattern
      , do
        opPattern <- addLocation (P.OpPattern <$> parens' symOp)
        func opPattern
      ]
    <?> "the definition of a variable (x = ...)"
 where
  func pattern = case pattern of
