  ...
  let blank   = string " " >> return []
      ...
      space =
          blank
            ...
            <|> comment
            <?> Syntax.whitespace
  in  concat <$> many1 space


forcedWS :: IParser Comments
forcedWS = choice
  [(++) <$> spaces <*> (concat <$> many nl_space), concat <$> many1 nl_space]
 where
  nl_space = try ((++) <$> (concat <$> many1 newline) <*> option [] spaces)
...
whitespace' = option (False, []) ((,) True <$> forcedWS)


whitespace :: IParser Comments
whitespace = snd <$> whitespace'
...
freshLine =
  concat
    ...
    <?> Syntax.freshLine
  where space_nl = try $ (++) <$> spaces <*> (concat <$> many1 newline)


newline :: IParser Comments
newline =
  (simpleNewline >> return [])
    <|> ((\x -> [x]) <$> lineComment)
    <?> Syntax.newline
...
restOfLine = many (char ' ') *> choice
  [ Just
  .   fst
  <$> (try (string "--") *> (anyUntil $ (lookAhead simpleNewline) <|> eof))
  , return Nothing
  ]
...
docCommentAsMarkdown = Markdown.parse <$> docComment


multiComment :: IParser Comment
multiComment = do
  _              <- try (string "{-" <* notFollowedBy (string "|"))
  isCommentTrick <- choice [char '-' >> return True, return False]
  _              <- many (string " ")
  b              <- closeComment False
  return $ if isCommentTrick
    then CommentTrickBlock b
    else BlockComment $ trimIndent $ lines b
 where
  trimIndent [] = []
  trimIndent (l1 : ls) =
    let leadingIndents = map fst $ filter (uncurry (/=)) $ map
          (\l -> (length $ takeWhile Char.isSpace l, length l))
          ls

        depth = case leadingIndents of
          [] -> 0
          _  -> minimum leadingIndents
    in  l1 : map (drop depth) ls


closeComment :: Bool -> IParser String
closeComment keepClosingPunc = uncurry (++) <$> anyUntil
  (choice
    [ try
        (   (\a b -> if keepClosingPunc then concat (a ++ [b]) else "")
        <$> many (string " ")
        <*> string "-}"
        )
      <?> "the end of a comment -}"
    , concat <$> sequence
      [try (string "{-"), closeComment True, closeComment keepClosingPunc]
    ]
  )
...
anyUntil end = go ""
 where
  ...
  go pre = ((,) (reverse pre) <$> end) <|> next pre
