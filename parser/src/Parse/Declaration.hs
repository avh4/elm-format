...
topLevelStructure entry =
  choice [DocComment <$> docCommentAsMarkdown, Entry <$> addLocation entry]
...
typeDecl elmVersion = do
  try (reserved elmVersion "type") <?> "a type declaration"
  postType                   <- forcedWS
  ...
  case isAlias of
    Just postAlias -> do
      ...
      return $ AST.Declaration.TypeAlias
        postType
    ...
    Nothing -> do
      tags_ <-
        pipeSep1 (Type.tag elmVersion) <?> "a constructor for a union type"
      return AST.Declaration.Datatype
...
infixDecl elmVersion = expecting "an infix declaration"
  $ choice [try $ infixDecl_0_16 elmVersion, infixDecl_0_19 elmVersion]
...
infixDecl_0_19 elmVersion =
  ...
  in  AST.Declaration.Fixity_0_19
...
infixDecl_0_16 elmVersion = do
  ...
  n             <- digit
  opComments    <- forcedWS
  AST.Declaration.Fixity assoc digitComments (read [n]) opComments
    <$> anyOp elmVersion
...
port elmVersion = expecting "a port declaration" $ do
  try (reserved elmVersion "port")
