...
import           Text.Parsec.Char                         ( octDigit
                                                          , space
                                                          , upper
                                                          )
import           Parse.Helpers                            ( processAs
                                                          , escaped
                                                          , expecting
                                                          , sandwich
                                                          , betwixt
...
literal = num <|> (uncurry Str <$> str) <|> (Chr <$> chr)


num :: IParser Literal
num = toLiteral <$> (rawNumber <?> "a number")


toLiteral :: String -> Literal
toLiteral n | 'x' `elem` n        = IntNum (read n) HexadecimalInt
...
rawNumber = concat <$> sequence [option "" minus, base16 <|> base10]
...
base10 =
  concat <$> sequence [many1 digit, option "" decimals, option "" exponent]
...
str = expecting "a string" $ do
  (s, multi) <- choice [multiStr, singleStr]
  result     <- processAs stringLiteral . sandwich '\"' $ concat s
  return (result, multi)
 where
  rawString quote insides = quote >> manyTill insides quote

  multiStr = do
    result <- rawString (try (string "\"\"\"")) multilineStringChar
    return (result, True)
  singleStr = do
    result <- rawString (char '"') stringChar
    return (result, False)
...
chr :: IParser Char
chr = betwixt '\'' '\'' character <?> "a character"
 where
  nonQuote  = satisfy (/= '\'')

  character = do
    c <- choice
      [escaped '\'', (:) <$> char '\\' <*> many1 nonQuote, (: []) <$> nonQuote]
...
charLiteral =
  lexeme (between (char '\'') (char '\'' <?> "end of character") characterChar)
    <?> "character"
...
charNum = do
  code <-
    ...
    <|> do
          _ <- char 'u'
          between (char '{') (char '}') (number 16 hexDigit)
  if code > 0x10FFFF
