...
iParse = iParseWithState "" State.init


iParseWithState
  :: SourceName -> State.State -> IParser a -> String -> Either ParseError a
...
innerVarChar elmVersion = if syntax_0_19_disallowApostropheInVars elmVersion
...
makeVar elmVersion firstChar = do
  variable <- (:) <$> firstChar <*> many (innerVarChar elmVersion)
...
reserved elmVersion word = expecting ("reserved word `" ++ word ++ "`") $ do
  _ <- string word
...
anyOp elmVersion =
  (   betwixt '`' '`' (qualifiedVar elmVersion)
...
symOp = do
  op <- many1 (satisfy Help.isSymbol) <?> "an infix operator like +"
...
lenientEquals = const () <$> (char '=' <|> char ':') <?> "="


rightArrow :: IParser ()
rightArrow = const () <$> (string "->" <|> string "\8594") <?> "->"


cons :: IParser ()
cons = const () <$> string "::" <?> "a cons operator '::'"


hasType :: IParser ()
hasType = const () <$> char ':' <?> "the \"has type\" symbol ':'"
...
comma :: IParser ()
comma = const () <$> char ',' <?> "a comma ','"


semicolon :: IParser ()
semicolon = const () <$> char ';' <?> "a semicolon ';'"


verticalBar :: IParser ()
verticalBar = const () <$> char '|' <?> "a vertical bar '|'"


commitIf :: IParser any -> IParser a -> IParser a
commitIf check p = commit <|> try p where commit = try (lookAhead check) >> p
...
spaceySepBy1 sep parser =
  let
      ...
      step first rest post = do
        next <- withEol parser
        choice
          [ try (padded sep)
            >>= (\(preSep, _, postSep) ->
                  step first (Commented post next preSep : rest) postSep
                )
...
spaceySepBy1''
  :: IParser sep
  -> IParser (Comments -> Comments -> a)
  -> IParser (Comments -> Comments -> [a])
spaceySepBy1'' sep parser =
  let combine eol post = Maybe.maybeToList (fmap LineComment eol) ++ post
  in  do
        ...
        case result of
          ...
          Multiple ((first, firstEol), postFirst) rest (preLast, (last, eol))
            -> return $ \preFirst postLast -> concat
              [ [first preFirst $ combine firstEol postFirst]
              , fmap
                (\(Commented pre (item, eol) post) ->
                  item pre $ combine eol post
                )
                rest
...
-- DEPRECATED: use spaceySepBy1 instead
spaceySepBy1'
  :: IParser sep -> IParser a -> IParser (Comments -> Comments -> [Commented a])
spaceySepBy1' sep parser =
...
toSet merge values =
  ...
  in  foldl
        (\m (Commented pre (k, v) post) ->
          insertWith merge' k (Commented pre v post) m
        )
        empty
        values
...
commaSep1Set'
  ...
  => IParser (k, v)
  -> (v -> v -> v)
  -> IParser (Comments -> Comments -> Map k (Commented v))
commaSep1Set' parser merge = do
...
commaSep
  :: IParser (Comments -> Comments -> a)
  -> IParser (Maybe (Comments -> Comments -> [a]))
commaSep term = option Nothing (Just <$> commaSep1 term)


pipeSep1 :: IParser a -> IParser (ExposedCommentedList a)
pipeSep1 = spaceySepBy1 verticalBar
...
keyValue
  ...
  -> IParser key
  -> IParser val
  -> IParser (Comments -> Comments -> (Commented key, Commented val))
keyValue parseSep parseKey parseVal = do
  key     <- parseKey
  preSep  <- whitespace <* parseSep
  postSep <- whitespace
  val     <- parseVal
  return $ \pre post -> (Commented pre key preSep, Commented postSep val post)
...
separated
  ...
  -> IParser
       ( Either
           ...
           ( R.Region
           , (e, Maybe String)
           , [(Comments, Comments, e, Maybe String)]
           , Bool
           )
       )
separated sep expr' =
  let subparser = do
        ...
        case arrow of
          Nothing                -> return $ \_multiline -> Left t1
          Just (eolT1, preArrow) -> do
            postArrow <- whitespace
            t2        <- separated sep expr'
            end       <- getMyPosition
            case t2 of
              Right (_, (t2', eolT2), ts, _) -> return $ \multiline -> Right
                ( R.Region start end
                , (t1, eolT1)
  ...
  in  (\(f, multiline) -> f $ multilineToBool multiline)
        <$> trackNewline subparser
...
spacePrefix p = fmap fst <$> constrainedSpacePrefix' p (\_ -> return ())


constrainedSpacePrefix :: IParser a -> IParser [((Comments, a), Multiline)]
constrainedSpacePrefix parser = constrainedSpacePrefix' parser constraint
 where
  constraint empty = if empty then notFollowedBy (char '-') else return ()


constrainedSpacePrefix'
  :: IParser a -> (Bool -> IParser b) -> IParser [((Comments, a), Multiline)]
constrainedSpacePrefix' parser constraint = many $ trackNewline $ choice
...
surround
  ...
  -> Char
  -> String
  -> IParser (Comments -> Comments -> Bool -> a)
  -> IParser a
surround a z name p =
  let
...
multilineToBool multine = case multine of
  SplitAll -> True
  JoinAll  -> False
...
surround''
  :: Char -> Char -> IParser a -> IParser (Either Comments [Commented a])
...
getMyPosition = R.fromSourcePos <$> getPosition
...
accessible
  :: ElmVersion -> IParser AST.Expression.Expr -> IParser AST.Expression.Expr
accessible elmVersion exprParser = do
  start <- getMyPosition
  ...
  case access of
    Nothing -> return annotatedRootExpr

    Just _  -> accessible elmVersion $ do
      v   <- lowVar elmVersion
      end <- getMyPosition
...
commentedKeyword
  :: ElmVersion -> String -> IParser a -> IParser (KeywordCommented a)
...
until p end = go where go = end <|> (p >> go)
...
processAs processor s = calloutParser s processor
 where
