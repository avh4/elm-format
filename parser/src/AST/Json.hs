...
pleaseReport what details = error
  (  "<elm-format-"
...
showModule (Module _ maybeHeader _ (_, imports) body) =
  let
    header = maybeHeader |> Maybe.fromMaybe AST.Module.defaultHeader

    (Header _ (Commented _ name _) _ _) = header

    getAlias :: ImportMethod -> Maybe UppercaseIdentifier
    getAlias importMethod = fmap (snd . snd) (alias importMethod)

    importAliases =
      Map.fromList $ map (\(k, v) -> (v, k)) $ Map.toList $ Map.mapMaybe
        (getAlias . snd)
        imports

    normalizeNamespace namespace = case namespace of
      [alias] -> Map.findWithDefault namespace alias importAliases
    ...
    importJson (moduleName, (_, ImportMethod alias (_, (_, exposing)))) =
      let
        name =
          List.intercalate "." $ fmap (\(UppercaseIdentifier n) -> n) moduleName
      in  ( name
          , makeObj
            [ ( "as"
              , JSString $ toJSString $ maybe
                name
                (\(_, (_, UppercaseIdentifier n)) -> n)
                alias
              )
...
instance ToJSON (List UppercaseIdentifier) where
  showJSON = JSString . toJSString . List.intercalate "." . fmap
    (\(UppercaseIdentifier v) -> v)


showImportListingJSON :: Listing DetailedListing -> JSValue
showImportListingJSON (ExplicitListing a _) = showJSON a
showImportListingJSON (OpenListing (Commented _ () _)) =
  JSString $ toJSString "Everything"
...
showTagListingJSON (ExplicitListing tags _) =
  makeObj
    $ fmap (\(UppercaseIdentifier k, Commented _ () _) -> (k, JSBool True))
    $ Map.toList tags
showTagListingJSON (OpenListing (Commented _ () _)) =
  JSString $ toJSString "AllTags"
showTagListingJSON ClosedListing = JSString $ toJSString "NoTags"


instance ToJSON DetailedListing where
  showJSON (DetailedListing values _operators types) = makeObj
    [ ( "values"
      , makeObj $ fmap (\(LowercaseIdentifier k) -> (k, JSBool True)) $ Map.keys
        values
      )
    , ( "types"
      , makeObj
      $ fmap
          (\(UppercaseIdentifier k, (Commented _ (_, listing) _)) ->
            (k, showTagListingJSON listing)
          )
      $ Map.toList types
      )
    ]
...
instance ToJSON Expr where
  showJSON (A region expr) = case expr of
    Unit                   _                   -> makeObj [type_ "UnitLiteral"]

    AST.Expression.Literal (IntNum value repr) -> makeObj
      [ type_ "IntLiteral"
      , ("value"  , JSRational False $ toRational value)
      , ("display", makeObj [("representation", showJSON repr)])
      ]

    AST.Expression.Literal (FloatNum value repr) -> makeObj
      [ type_ "FloatLiteral"
      , ("value"  , JSRational False $ toRational value)
      , ("display", makeObj [("representation", showJSON repr)])
      ]

    AST.Expression.Literal (Boolean value) -> makeObj
    ...
    AST.Expression.Literal (Chr chr) -> makeObj
      [ type_ "CharLiteral"
    ...
    AST.Expression.Literal (Str str _) -> makeObj
      [ type_ "StringLiteral"
    ...
    VarExpr (VarRef namespace (LowercaseIdentifier var)) -> makeObj
      [ type_ "ExternalReference"
    ...
    VarExpr (OpRef (SymbolIdentifier sym)) -> variableReference region sym

    VarExpr _ -> JSString $ toJSString "TODO: VarExpr"

    App expr args _ -> makeObj
      [ type_ "FunctionApplication"
      , ("function" , showJSON expr)
      , ("arguments", JSArray $ showJSON <$> map (\(_, arg) -> arg) args)
      ]

    Binops first rest _ -> makeObj
    ...
    Unary Negative expr -> makeObj
      [ type_ "UnaryOperator"
      , ("operator", JSString $ toJSString "-")
      , ("term"    , showJSON expr)
      ]

    Parens (Commented _ expr _) -> showJSON expr

    ExplicitList terms _ _      -> makeObj
      [ type_ "ListLiteral"
      , ( "terms"
        , JSArray $ fmap showJSON (map (\(_, (_, (term, _))) -> term) terms)
        )
      ]

    AST.Expression.Tuple exprs _ -> makeObj
      [ type_ "TupleLiteral"
      , ( "terms"
        , JSArray $ fmap showJSON (map (\(Commented _ expr _) -> expr) exprs)
        )
      ]
    ...
    TupleFunction n -> variableReference region $ replicate (n - 1) ','
    ...
    AST.Expression.Record base fields _ _ ->
      let
        ...
        fieldOrder =
          ( "fieldOrder"
          , JSArray $ fmap (JSString . toJSString) $ fmap
            (\(_, (_, (Pair (LowercaseIdentifier key, _) _ _, _))) -> key)
            fields
          )
      in
        case base of
          Just (Commented _ (LowercaseIdentifier identifier) _) -> makeObj
            [ type_ "RecordUpdate"
            , ("base", JSString $ toJSString identifier)
            , fieldsJSON
            , ("display", makeObj [fieldOrder])
            ]
    ...
    Access base (LowercaseIdentifier field) -> makeObj
      [ type_ "RecordAccess"
      , ("record", showJSON base)
      , ("field" , JSString $ toJSString field)
      ]

    Lambda parameters _ body _ -> makeObj
    ...
    If (Commented _ cond' _, Commented _ thenBody' _) rest' (_, elseBody) ->
      let
        ...
        ifThenElse cond thenBody rest = makeObj
          [ type_ "IfExpression"
          , ("if"  , showJSON cond)
    ...
    Let decls _ body -> makeObj
      [ type_ "LetExpression"
      , ("declarations", JSArray $ map showJSON decls)
      , ("body"        , showJSON body)
      ]

    Case (Commented _ subject _, _) branches -> makeObj
      [ type_ "CaseExpression"
      , ("subject", showJSON subject)
      , ( "branches"
        , JSArray $ map
          (\(Commented _ (A _ pat) _, (_, body)) ->
            makeObj [("pattern", showJSON pat), ("body", showJSON body)]
          )
          branches
        )
      ]

    Range _ _ _      -> JSString $ toJSString "TODO: Range"

    AccessFunction _ -> JSString $ toJSString "TODO: AccessFunction"

    GLShader       _ -> JSString $ toJSString "TODO: GLShader"


variableReference :: Region.Region -> String -> JSValue
variableReference region name = makeObj
...
instance ToJSON LetDeclaration where
  showJSON letDeclaration = case letDeclaration of
...
type_ t = ("type", JSString $ toJSString t)
