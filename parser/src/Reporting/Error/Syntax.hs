...
toReport err = case err of
  Parse          messages -> parseErrorReport messages

  InfixDuplicate op       -> Report.simple
    "INFIX OVERLAP"
  ...
  TypeWithoutDefinition valueName -> Report.simple
    "MISSING DEFINITION"
  ...
  PortWithoutAnnotation portName -> Report.simple
    "PORT ERROR"
  ...
  DuplicateValueDeclaration name -> Report.simple
  ...
  DuplicateTypeDeclaration name -> Report.simple
    "DUPLICATE DEFINITION"
    (  "Naming multiple types `"
    ++ name
    ...
    (  "Find all the types named `"
    ++ name
    ++ "` and\n"
    ++ "do some renaming. Make sure the names are distinct!"
    )

  DuplicateDefinition name -> Report.simple
    "DUPLICATE DEFINITION"
    (  "Naming multiple values `"
    ++ name
    ++ "` in a single let-expression makes\n"
    ++ "things ambiguous. When you say `"
    ++ name
    ++ "` which one do you want?"
    )
    (  "Find all the values named `"
    ++ name
    ++ "` in this let-expression and\n"
    ++ "do some renaming. Make sure the names are distinct!"
    )
...
keyword kwd = "KEYWORD=" ++ kwd

unkeyword :: String -> Maybe String
unkeyword message = if List.isPrefixOf "KEYWORD=" message
...
parseErrorReport messages =
  let addMsg message hint = case message of
        Parsec.SysUnExpect _msg -> hint

        Parsec.UnExpect    _msg -> hint

        Parsec.Expect msg ->
          let msg' = if msg `elem` [whitespace, newline, freshLine]
                then "whitespace"
                else msg
          in  hint { _expected = Set.insert msg' (_expected hint) }

        Parsec.Message msg -> hint { _messages = msg : _messages hint }

      (ParseHint msgs expects) = foldr addMsg emptyHint messages

      preHint                  = case msgs of
        [msg] -> case unkeyword msg of
          Just kwd ->
      ...
      postHint = if Set.null expects
        then ""
        else "I am looking for one of the following things:\n"
          ++ concatMap ("\n    " ++) (Set.toList expects)
  in  Report.simple "SYNTAX PROBLEM" preHint postHint
