...
throw region err = Result [] (Err [A.A region err])


throwMany :: [A.Located e] -> Result w e a
throwMany errors = Result [] (Err errors)


from :: (e -> e') -> Except [A.Located e] a -> Result w e' a
from f except = case runExcept except of
  Right answer -> ok answer

  Left  errors -> throwMany (map (A.map f) errors)


mapError :: (e -> e') -> Result w e a -> Result w e' a
mapError f (Result warnings rawResult) = Result warnings $ case rawResult of
  Ok  v    -> Ok v

  Err msgs -> Err (map (A.map f) msgs)


warn :: R.Region -> w -> Result w e ()
warn region warning = Result [A.A region warning] (Ok ())
...
destruct errFunc okFunc rawResult = case rawResult of
  Ok  answer -> okFunc answer

  Err errors -> errFunc errors
...
instance M.Functor (Result w e) where
  fmap func (Result warnings rawResult) = case rawResult of
    Ok  a    -> Result warnings (Ok (func a))

    Err msgs -> Result warnings (Err msgs)
...
instance Applicative (Result w e) where
  ...
  (<*>) (Result warnings resultFunc) (Result warnings' resultVal) =
    Result (warnings ++ warnings') $ case (resultFunc, resultVal) of
      (Ok  func, Ok val   ) -> Ok (func val)

      (Err msgs, Err msgs') -> Err (msgs ++ msgs')
...
instance M.Monad (Result w e) where
  ...
  (>>=) (Result warnings rawResult) callback = case rawResult of
    Err msg -> Result warnings (Err msg)

    Ok value ->
      let (Result warnings' rawResult') = callback value
      in  Result (warnings ++ warnings') rawResult'
